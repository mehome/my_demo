...
//#endif
	...
	{11, 0x11000000, 0},
	{12, 0x40004820, 0},
#if 1 //(CONFIG_PANTHER_CHIP_VERSION >= 1)
	{13, 0x73495BBB, 0},
	{14, 0x0073F108, 0},
	{15, 0x0091AFAD, 0},
	{16, 0x5E2296, 0},
...
int lrf_lpf_cal(void)
{
	...
	val = (0x3 << 5) | 0x2;	// Addr28[1] = b'1ï¼ŒAddr28[6:5] = b'11ï¼ŒAddr28[8:7] = b'00
	mask = (0xf << 5) | 0x2;
...
//      volatile u32 *ptr;
	...
	int t_val = 0;
#endif					
	// step0: Initial Setting ï¼ˆthe setting as following will do one time by soft ware after power on for chipï¼?
...
//      ptr = (u32*)0xC085C;			//SDIO_CTRL_REG
//      *ptr = *ptr & (~(1 << 12));		// Reg085C[12]=0(RF_pd = 0ï¼?
...
int panther_rf_test_cmd(int argc, char *argv[])
{
	unsigned char *str;
...
//   	WLAN_DBG("    rf_on <is_tx: 2=txrx, 1=tx, 0=rx>\n");
//   	WLAN_DBG("    iq_swap <sel: 0=disable, 1=enable, others=show >\n");
//   	WLAN_DBG("    lpf_cal\n");
//   	WLAN_DBG("    channel_pll_cal (scan all 14 channels' pll)\n");
//   	WLAN_DBG("    set_channel <ch_num: 1~14>\n");
//   	WLAN_DBG("    channel_pll_test <ch_num: 1~14> <test loop>\n");
//#ifdef CONFIG_TX_PWR_CMD	// for test (iot_boot proj won't use this)
//        WLAN_DBG("    set_tx_pwr <level: 0~15>\n");
//#endif
//        WLAN_DBG("    driver_boost <enable>\n");
//        WLAN_DBG("    voltage_temperature <delay_vot> <delay_reg9> <dbg_en>\n");
		return 0;
	}
...
//  		lrf_rf_on(is_tx);
//  }
//  else if(!strcmp(str, "channel_pll_cal"))
//  {
//  	lrf_channel_pll_cal(0);
//  }
//  else if(!strcmp(str, "set_channel"))
//  {
//  	int channel=7, idx=0;
//  	if(argc >= 2)
//  	    channel = simple_strtol(argv[1], NULL, 10);
//
//  	idx = lrf_chan2idx(channel);
//  	lrf_set_freq(lrf_channel_data[idx].freq);
//  }
//  else if(!strcmp(str, "channel_pll_test"))
//  {
//  	int channel=1, idx=0, loop=0;
//
//  	if(argc >= 2)
//  	    simple_strtol(argv[1], NULL, 10);
//  	if(argc >= 3)
//  	    simple_strtol(argv[2], NULL, 10);
//
//  	idx = lrf_chan2idx(channel);
//
//  	do {
//  		lrf_channel_pll_cal(lrf_channel_data[idx].freq);
//  		lrf_set_freq(lrf_channel_data[idx].freq);
//  		rf_update(1, 0x1, 0x1);		// reg_ctrl_mode: manual mode
//   		WLAN_DBG("channel_pll_test loop = %d\n", loop);
//  		//WLA_GETS(buf);
//  	} while(loop-- > 0);
//  }
//#ifdef CONFIG_TX_PWR_CMD	// for test
//    else if(!strcmp(str, "set_tx_pwr"))
//    {
//        int level=0;
//
//        if(argc >= 2)
//            level = simple_strtol(argv[1], NULL, 10);
//
//        wmac_set_txpwr(level);
//    }
//#endif
//  else if(!strcmp(str, "lpf_cal"))
//  {
//  	lrf_lpf_cal();
//  }
//  else if(!strcmp(str, "voltage_temperature"))
//  {
//  	int delay_vot=1000, delay_reg9=10, dbg_en=0;
//  	if(argc >= 2)
//  	    delay_vot = simple_strtol(argv[1], NULL, 10);
//  	if(argc >= 3)
//  	    delay_reg9 = simple_strtol(argv[2], NULL, 10);
//  	if(argc >= 3)
//  	    dbg_en = simple_strtol(argv[3], NULL, 10);
//
//  	WLAN_DBG("voltage_temperature: delay_vot=%d, delay_reg9=%d\n", delay_vot, delay_reg9);
//  	lrf_voltage_temperature(delay_vot, delay_reg9, dbg_en);
//  }
	else
	{
...
#if 0
int bb_dump_cmd(int argc, char *argv[])
{
    unsigned long byte_count = 0x1000000;
    unsigned int buf = 0x82000000UL;
    unsigned int merge_buf = 0x83000000UL;
