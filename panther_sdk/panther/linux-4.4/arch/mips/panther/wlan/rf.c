...
volatile int wifimac_gated;
#endif
void rf_power_on(void)
{
	u32 val;
	DRG_NOTICE(DRAGONITE_DEBUG_PS_FLAG, "RF power-on\n");

#if defined(DRAGONITE_POWERSAVE_AGGRESSIVE)
	/* enable RF power */
	val = *(volatile u32*)0xbf00480c;
	val &= ~(0x00800000UL);
	*(volatile u32*)0xbf00480c = val;

	/* enable BB clock */
	val = *(volatile u32*)0xbf004a58;
	val |= (0x0020UL);
	*(volatile u32*)0xbf004a58 = val;

	/* enable WIFI MAC clock */
	val = *(volatile u32*)0xbf004a58;
	val |= (0x0010UL);
	*(volatile u32*)0xbf004a58 = val;
	...
	val &= ~(0x02000000UL);
	*(volatile u32*)0xbf00480c = val;
	//*(volatile u32*)0xbf0048f8 = 0x1;
}

void rf_power_off(void)
{
	u32 val;
	DRG_NOTICE(DRAGONITE_DEBUG_PS_FLAG, "RF power-off\n");

	/* RF poweroff (LMAC TIMER also control this) */
	//*(volatile u32*)0xbf0048f8 = 0x3;
...
void rf_power_off_aggressive(void)
{
	u32 val;
