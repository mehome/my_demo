...
static u8 bb_regs[TOTAL_REG_CNT];
void dragonite_bb_suspend(void)
{
    backup_bb_result(bb_regs);
}

void dragonite_bb_resume(void)
{
    struct mac80211_dragonite_data *data = global_data;
    unsigned int channel, bandwidth;

    restore_bb_result(bb_regs);
    bb_init();
    ...
    dragonite_set_channel(channel, bandwidth);
}
