...
#if 0   //PCM time slot mode
	SOC_SINGLE("PCM Time Slot Mode Switch", 0x3c, 8, 1, 0),
	SOC_SINGLE("PCM Left Channel Slot ID[9]", 0x3c, 0, 1, 0),
	SOC_SINGLE("PCM Right Channel Slot ID[9]", 0x3c, 1, 1, 0),
	SOC_SINGLE("PCM Left Channel Slot ID[0-8]", 0x3b, 0, 0x1ff, 0),
	SOC_SINGLE("PCM Right Channel Slot ID[0-8]", 0x3d, 0, 0x1ff, 0),
#endif
};
...
static const struct snd_kcontrol_new nau8822_left_out_mixer[] = {
	...
	SOC_DAPM_SINGLE("Left Playback Switch", NAU8822_LEFT_MIXER_CONTROL, 0, 1, 0),
};
...
static const struct snd_kcontrol_new nau8822_right_out_mixer[] = {
	...
	SOC_DAPM_SINGLE("Right Playback Switch", NAU8822_RIGHT_MIXER_CONTROL, 0, 1, 0),
};
...
static int nau8822_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	dev_dbg(codec->dev, "%s\n", __func__);
	pr_debug("in %s, origin iface %x, clk %x\n", __func__, iface, clk);
	...
	pr_debug("write iface %x, clk %x to codec\n", iface, clk);
			    ...
			    struct snd_soc_dai *dai)
{
	...
	int i, best = 0;
	pr_debug("in %s, iface %x, add_ctl %x, clking %x\n", __func__, iface_ctl, add_ctl, clking);
	if (!nau8822->f_mclk)
		return -EINVAL;
...
//	snd_soc_update_bits(codec, NAU8822_CLOCKING, 0xe0, best << 5);
	/* Cheetah gives exact 256fs MCLK, therefore codec no need to divide MCLK */
	snd_soc_update_bits(codec, NAU8822_CLOCKING, 0xe0, 0); 
	pr_debug("write codec iface %x, add_ctl %x\n", iface_ctl, add_ctl);
...
static int nau8822_mute(struct snd_soc_dai *dai, int mute)
{
	...
	pr_debug("in mute codec reg value\n");
	for (i = 5; i < ARRAY_SIZE(nau8822_reg); i+=5){
		pr_debug("reg %d: %3x %3x %3x %3x %3x", i, snd_soc_read(codec, i-4), snd_soc_read(codec, i-3), snd_soc_read(codec, i-2), 
						snd_soc_read(codec, i-1), snd_soc_read(codec, i));
	}
	
//	printk(KERN_EMERG "reg 3c : %x reg 3b : %x reg 3d : %x\n", snd_soc_read(codec, 0x3c), snd_soc_read(codec, 0x3b), snd_soc_read(codec, 0x3d));
	return 0;
}
...
#define NAU8822_FORMATS (SNDRV_PCM_FMTBIT_S16_BE | SNDRV_PCM_FMTBIT_S20_3LE | \
...
//	nau8822->sysclk = NAU8822_PLL;
	codec->control_data = nau8822->control_data;
	...
	/* enable nau8822's PLL, force codec startup, prevent some function don't work until DAC/ADC enable */
	snd_soc_update_bits(codec, NAU8822_POWER_MANAGEMENT_1, 0x20, 0x20);
	nau8822_set_bias_level(codec, SND_SOC_BIAS_STANDBY);
	/* once enable PLL, we could disable PLL if we apply externel MCLK as clock source */
	snd_soc_update_bits(codec, NAU8822_POWER_MANAGEMENT_1, 0x20, 0x0);	
	return 0;
}
