...
static int es7210_read(u8 reg, u8 *rt_value, struct i2c_client *client)
{
	...
	if (rs != 2 || rr != 0) {
		pr_err("%i: es7210_write[%x] error, REG[0x%x].\n", i, client->addr, reg);
	}
...
static int es7210_write(u8 reg, unsigned char value, struct i2c_client *client)
{
	...
	for (i = 0; i < ES7210_MAX_RETRY; i++) {
		...
		if (rs == 2) {
			...
			if( client->addr == 0x40 && reg == ES7210_RESET_CTL_REG00 )
				return 0;

			rr = es7210_read(reg, &rt_value, client);
			if (rr == 0 && value == rt_value) {
				pr_debug("%d: es7210_write[%x] OK, REG[0x%x] rs = %d.\n", i, client->addr, reg, rs);
				return 0;
			}
			else {
				pr_err("%d: es7210_write[%x] error, REG[0x%x] rs = %d.\n", i, client->addr, reg, rs);
			}
	...
	int clk_id, unsigned int freq, int dir)
{
	...
	pr_debug("es7210_set_dai_sysclk\n");
...
static void es7210_tdm_init_codec(u8 mode)
{
	...
	case ES7210_TDM_1LRCK_DSPA:
		...
		*/
		for (i = 0; i < ADC_DEV_MAXNUM; i++) {
			es7210_write(ES7210_SDP_CFG1_REG11, 0x63, i2c_clt1[i]);
			es7210_write(ES7210_SDP_CFG2_REG12, 0x01, i2c_clt1[i]);
		}
		break;
	case ES7210_TDM_1LRCK_DSPB:
		...
		*/
		for (i = 0; i < ADC_DEV_MAXNUM; i++) {
			es7210_write(ES7210_SDP_CFG1_REG11, 0x73, i2c_clt1[i]);
			es7210_write(ES7210_SDP_CFG2_REG12, 0x01, i2c_clt1[i]);
		}
	...
	case ES7210_TDM_1LRCK_I2S:
		...
		*/
		for (i = 0; i < ADC_DEV_MAXNUM; i++) {
			es7210_write(ES7210_SDP_CFG1_REG11, 0x60, i2c_clt1[i]);
			es7210_write(ES7210_SDP_CFG2_REG12, 0x02, i2c_clt1[i]);
		}
	...
	case ES7210_TDM_1LRCK_LJ:
		...
		*/
		for (i = 0; i < ADC_DEV_MAXNUM; i++) {
			es7210_write(ES7210_SDP_CFG1_REG11, 0x61, i2c_clt1[i]);
			es7210_write(ES7210_SDP_CFG2_REG12, 0x02, i2c_clt1[i]);
		}
		break;
		...
		*/
		for (i = 0; i < ADC_DEV_MAXNUM; i++) {
			...
			if (i == 0) {
				...
				*/
				es7210_write(ES7210_SDP_CFG2_REG12, 0x07, i2c_clt1[i]);
			}
			else {
				es7210_write(ES7210_SDP_CFG2_REG12, 0x03, i2c_clt1[i]);
			}
		}
