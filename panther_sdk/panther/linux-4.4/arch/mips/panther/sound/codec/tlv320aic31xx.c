...
static const struct aic31xx_rate_divs aic31xx_divs[] = {
	...
	/* 8k rate */
    { 2048000,   8000, 48,    0,	128, 48,   2,	128, 48,   2},
	/* 16k rate */
    { 4096000,  16000, 24,    0,	128, 24,   2,	128, 24,   2},
	/* 22.05k rate */
    { 5644800,  22050, 16,    0,	128,  8,   4,	128,  8,   4},
	/* 32k rate */
    { 8192000,  32000, 12,    0,	128, 12,   2,	128, 12,   2},
	/* 44.1k rate */
	...
	/* 176.4k rate */
    {15052800, 176400,  6,    0,	 32,  8,   2,	 32,  8,   2},
	/* 192k rate */
    {16384000, 192000,  6,    0,	 32,  8,   2,	 32,  8,   2},
	#else
	/* mclk/p    rate  pll: j     d        dosr ndac mdac  aors nadc madc */
	/* 8k rate */
...
static void aic31xx_reset(struct snd_soc_codec *codec)
{
    // PAGE 0
    snd_soc_write(codec, AIC31XX_RESET,     0x01);

    snd_soc_write(codec, AIC31XX_CLKMUX,    0x03);
    snd_soc_write(codec, AIC31XX_PLLPR,     0x91);
    mdelay(10);
    snd_soc_write(codec, AIC31XX_PLLJ,      0x08);
    snd_soc_write(codec, AIC31XX_PLLDMSB,   0x00);
    snd_soc_write(codec, AIC31XX_PLLDLSB,   0x00);
    snd_soc_write(codec, AIC31XX_IFACE1,    0x00);
    snd_soc_write(codec, AIC31XX_NDAC,      0x84);
    snd_soc_write(codec, AIC31XX_MDAC,      0x84);
    snd_soc_write(codec, AIC31XX_DOSRMSB,   0x00);
    snd_soc_write(codec, AIC31XX_DOSRLSB,   0x80);
    snd_soc_write(codec, AIC31XX_LDACVOL,   0x24);
    snd_soc_write(codec, AIC31XX_RDACVOL,   0x24);

    snd_soc_write(codec, AIC31XX_NADC,      0x84);
    snd_soc_write(codec, AIC31XX_MADC,      0x84);
    snd_soc_write(codec, AIC31XX_AOSR,      0x00);
    snd_soc_write(codec, AIC31XX_IADC_VAL,  0x80);

    // PAGE 1
    snd_soc_write(codec, AIC31XX_HPPOP,         0x4e);
    snd_soc_write(codec, AIC31XX_HPDRIVER,      0xc2);
    snd_soc_write(codec, AIC31XX_DACMIXERROUTE, 0x44);
    snd_soc_write(codec, AIC31XX_HPLGAIN,       0x06);
    snd_soc_write(codec, AIC31XX_HPRGAIN,       0x06);
    snd_soc_write(codec, AIC31XX_SPLGAIN,       0x1c);
    snd_soc_write(codec, AIC31XX_SPRGAIN,       0x1c);
    snd_soc_write(codec, AIC31XX_SPKAMP,        0xc6);
    snd_soc_write(codec, AIC31XX_MICBIAS,       0x0b);
    snd_soc_write(codec, AIC31XX_MICPGAPI,       0x40);
    snd_soc_write(codec, AIC31XX_MICPGAMI,       0x40);

    // PAGE 0
    snd_soc_write(codec, AIC31XX_DACPRB,        0x0b);
    snd_soc_write(codec, AIC31XX_DACSETUP,      0xd6);
    snd_soc_write(codec, AIC31XX_ADCSETUP,      0x80);
    snd_soc_write(codec, AIC31XX_ADCFGA,        0x00);

    // PAGE 1
    snd_soc_write(codec, AIC31XX_DACMIXERROUTE, 0x44);
    snd_soc_write(codec, AIC31XX_LANALOGHPL,    0x00);
    snd_soc_write(codec, AIC31XX_RANALOGHPR,    0x00);
    snd_soc_write(codec, AIC31XX_LANALOGSPL,    0x80);
    snd_soc_write(codec, AIC31XX_RANALOGSPR,    0x80);
    snd_soc_write(codec, AIC31XX_SPLGAIN,       0x0d);
    snd_soc_write(codec, AIC31XX_SPRGAIN,       0x0d);
    snd_soc_write(codec, AIC31XX_SPLGAIN,       0x1c);
    snd_soc_write(codec, AIC31XX_SPRGAIN,       0x1c);
    snd_soc_write(codec, AIC31XX_SPKAMP,        0xc6);
}
...
static int aic31xx_add_widgets(struct snd_soc_codec *codec)
{
	...
	if (aic31xx->pdata.codec_type & AIC31XX_STEREO_CLASS_D_BIT) {
        //printk(KERN_EMERG "##%%%% aic311x_dapm_widgets add routes\n");
		ret = snd_soc_dapm_new_controls(
			dapm, aic311x_dapm_widgets,
			ARRAY_SIZE(aic311x_dapm_widgets));
		if (ret)
			return ret;
        //printk(KERN_EMERG "##%%%% aic311x_audio_map add routes\n");
		ret = snd_soc_dapm_add_routes(dapm, aic311x_audio_map,
					      ARRAY_SIZE(aic311x_audio_map));
		if (ret)
			return ret;
	} else {
	    //printk(KERN_EMERG "##%%%% aic310x_dapm_widgets add routes\n");
		ret = snd_soc_dapm_new_controls(
			dapm, aic310x_dapm_widgets,
			ARRAY_SIZE(aic310x_dapm_widgets));
		if (ret)
			return ret;
        //printk(KERN_EMERG "##%%%% aic310x_audio_map add routes\n");
			     ...
			     struct snd_pcm_hw_params *params)
{
	...
	int match = -1;
	int i;
    //printk(KERN_EMERG "aic31xx_setup_pll\n");
    ...
    //printk(KERN_EMERG "##match %d, bclk_n %d, bclk_socre %d\n", match, bclk_n, bclk_score);
...
static void aic31xx_clk_on(struct snd_soc_codec *codec)
{
    ...
    //printk("nadc %d", aic31xx_divs[aic31xx->rate_div_line].nadc);
    //printk("madc %d", aic31xx_divs[aic31xx->rate_div_line].madc);
...
#define TLV320_RATES (SNDRV_PCM_RATE_8000 | SNDRV_PCM_RATE_11025 |\
...
#define TLV320_FORMATS (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S20_3LE |\
...
static int __init aic31xx_modinit(void)
{
    int ret = 0;

    ret = i2c_add_driver(&aic31xx_i2c_driver);
    if (ret != 0) {
        printk(KERN_ERR "Failed to register tlv320 I2C driver: %d\n",
               ret);
    }
    return ret;
}
module_init(aic31xx_modinit);

static void __exit aic31xx_exit(void)
{
    i2c_del_driver(&aic31xx_i2c_driver);
}
module_exit(aic31xx_exit);
