...
#else
#define ES7243_CHANNELS_MAX         4
#endif
#define ES7243_I2C_BUS_NUM          0
#define ES7243_CODEC_RW_TEST_EN     0
#define ES7243_IDLE_RESET_EN        1       //reset AC108 when in idle time
#define ES7243_MATCH_DTS_EN         0       //AC108 match method select: 0: i2c_detect, 1:of_device_id

#define DEFAULT_GAIN_IDX 7
#define EVEREST_SUGGESTION 1                // define it for enable the suggestion about initial sequence (for cchip module) from Everest,
                                            // otherwise, using original sequence from Everest
#define SUPPORT_4096MHZ 1
#define SUPPORT_6144MHZ 1
...
#define pr_debug(fmt, ...) \
    printk(KERN_DEBUG purin_fmt(fmt), ##__VA_ARGS__)
#endif
...
/*
 * this part for initialize, it will call soc_dapm_update_bits in soc-dapm.c
 */
static const struct snd_soc_dapm_widget es7243_dapm_widgets[] = {
    SND_SOC_DAPM_INPUT("Line Input 1"),

    SND_SOC_DAPM_PGA("Line Input Switch", ES7243_ANACTL1_REG08, 0, 0, NULL, 0),
...
static int es7243_read(u8 reg, u8 *rt_value, struct i2c_client *client)
{
	...
	for (i = 0; i < ES7243_MAX_RETRY; i++) {
		rs = i2c_master_send(client, read_cmd, cmd_len);
		if (rs == cmd_len) {
			pr_debug("%d: i2c_sendr[%x] OK, REG[0x%x] rs = %d.\n", i, client->addr, reg, rs);
			rr = i2c_master_recv(client, rt_value, 1);
			if (rr != 1) {
				pr_err("%d: i2c_recv[%x] error, REG[0x%x] rr = %d.\n", i, client->addr, reg, rr );

				continue;
			}
			else if (rr == 1) {
				pr_debug("%d: i2c_recv[%x] OK, REG[0x%x] rr = %d.\n", i, client->addr, reg, rr );
				return 0;
			}
		} else {
			pr_err("%d: i2c_sendr[%x] error, REG[0x%x] rs = %d.\n", i, client->addr, reg, rs );

		}
	}
	if( rs != 2 || rr != 0 ) {
		pr_err("%i: es7243_write[%x] error, REG[0x%x].\n", i, client->addr, reg);
	}
...
static int es7243_write(u8 reg, unsigned char value, struct i2c_client *client)
{
	...
	for (i = 0; i < ES7243_MAX_RETRY; i++) {
		rs = i2c_master_send(client, write_cmd, 2);
		if (rs == 2) {
			pr_debug("%d: i2c_sendw[%x] OK, REG[0x%x] rs = %d.\n", i, client->addr, reg, rs);

			if (reg == ES7243_MUTECTL_REG05 )
				return 0;

			rr = es7243_read(reg, &rt_value, client);
			if (rr == 0 && value == rt_value) {
				pr_debug("%d: es7243_write[%x] OK, REG[0x%x] rs = %d.\n", i, client->addr, reg, rs);
				return 0;
			}
			else {
				pr_err("%d: es7243_write[%x] error, REG[0x%x] rs = %d.\n", i, client->addr, reg, rs);
			}

		}	else {
			pr_err("%d: i2c_sendw[%x] error, REG[0x%x] rs = %d.\n", i, client->addr, reg, rs);
...
static int es7243_update_bits(u8 reg, u8 mask, u8 value, struct i2c_client *client)
{
    ...
    if(client)
        pr_debug("es7243_update_bits[%x] %x, %x, %x \n", client->addr, reg, mask, value);
...
static const struct _coeff_div coeff_div[] = {
    ...
    /* 4.096 MHz for PCM */
    { 4096000, 16000 , 0, 0x02 , 0x10, 2 , 32},
#endif
#ifdef SUPPORT_6144MHZ
    /* 6.144 MHz for PCM */
    { 6144000, 16000 , 0, 0x03 , 0x18, 4 , 32},
#endif
    /* 8.192 MHz for PCM*/
    { 8192000, 16000 , 0, 0x04 , 0x20, 2 , 32},
    { 8192000, 32000 , 0, 0x02 , 0x10, 2 , 32},
    /* 12.288MHZ */
    ...
    {12288000, 32000 , 0, 0x03 , 0x18, 3 , 32},
        ...
        int clk_id, unsigned int freq, int dir)
{
    ...
    pr_debug("set dai sysclk %d\n", freq);
    switch (freq) {
#ifdef SUPPORT_4096MHZ
    case 4096000:
        es7243->sysclk_constraints = &constraints_4096;
        es7243->sysclk = freq;
        return 0;
#endif
#ifdef SUPPORT_6144MHZ
    case 6144000:
        es7243->sysclk_constraints = &constraints_6144;
        es7243->sysclk = freq;
        return 0;
#endif
    case 8192000:
        es7243->sysclk_constraints = &constraints_8192;
        es7243->sysclk = freq;
        return 0;
    case 11289600:
    case 22579200:
...
{
    pr_debug("es7243_init_codec \n");
#ifdef EVEREST_SUGGESTION
    es7243_write(ES7243_MODECFG_REG00,  0x01, i2c_clt[i]); //slave mode, software mode
    es7243_write(ES7243_STATECTL_REG06, 0x18, i2c_clt[i]); // bit[3] Reset adc logic, bit[4] reset all digital logic except cp logic
    es7243_write(ES7243_ANACTL0_REG07,  0x20, i2c_clt[i]); // power down ADCVREF generate module
    es7243_write(ES7243_ANACTL2_REG09,  0x80, i2c_clt[i]); // power down analog control
    es7243_write(ES7243_SDPFMT_REG01,   0xCF, i2c_clt[i]); // DSP-A, TDM ENABLE, BCLKINV
    es7243_write(ES7243_MUTECTL_REG05,  0x1B, i2c_clt[i]); // Mute ADC

    //es7243_write(ES7243_ANACTL1_REG08,  es7243_gain_sel_table[gain_ctrl_idx[i]] | 0x1, i2c_clt[i]);
	if (i == 0) {
		es7243_write(ES7243_ANACTL1_REG08,  es7243_gain_sel_table[7] | 0x1, i2c_clt[i]);
	}
	else {
		es7243_write(ES7243_ANACTL1_REG08,  es7243_gain_sel_table[2] | 0x1, i2c_clt[i]);
	}
    es7243_write(ES7243_ANACTL2_REG09,  0x3F, i2c_clt[i]); // enter low power for analog control
    es7243_write(ES7243_ANACTL0_REG07,  0x80, i2c_clt[i]); // power on ADCVREF and return Vmid to default value
    es7243_write(ES7243_STATECTL_REG06, 0x00, i2c_clt[i]); // return to default value
    es7243_write(ES7243_MUTECTL_REG05,  0x13, i2c_clt[i]); // unMute ADC
	es7243_update_bits(ES7243_ANACTL1_REG08, 0x08, 0x08, i2c_clt[i]);
#else
    es7243_write(ES7243_MODECFG_REG00,  0x01, i2c_clt[i]); //slave mode, software mode
    es7243_write(ES7243_STATECTL_REG06, 0x00, i2c_clt[i]);
    es7243_write(ES7243_MUTECTL_REG05,  0x1B, i2c_clt[i]); //Mute ADC
    es7243_write(ES7243_SDPFMT_REG01,   0x8F, i2c_clt[i]); //DSP-A, TDM ENABLE
    es7243_write(ES7243_ANACTL1_REG08,  es7243_gain_sel_table[gain_ctrl_idx[i]] | 0x1, i2c_clt[i]);
    es7243_write(ES7243_MUTECTL_REG05,  0x13, i2c_clt[i]); //un Mute ADC
#endif
}
#if 0
static void es7243_init_codec(struct snd_soc_codec *codec, struct i2c_client *i2c)
{
    pr_debug("es7243_init_codec \n");
    es7243_write(ES7243_MODECFG_REG00,  0x01, i2c); //slave mode, software mode
    es7243_write(ES7243_STATECTL_REG06, 0x00, i2c);
    es7243_write(ES7243_MUTECTL_REG05,  0x1B, i2c); //Mute ADC
    es7243_write(ES7243_SDPFMT_REG01,   0x8F, i2c); //DSP-A, TDM ENABLE
    es7243_write(ES7243_ANACTL1_REG08,  0x43, i2c); //enable AIN, PGA GAIN = 27DB
    es7243_write(ES7243_MUTECTL_REG05,  0x13, i2c); //un Mute ADC
}
#endif
                ...
                struct snd_soc_dai *dai)
{
    ...
    for(i=0; i<(ES7243_CHANNELS_MAX)/2; i++){
        ...
        pr_debug("es7243->sysclk %d\n", es7243->sysclk);
        //es7243_set_dai_sysclk(dai, 0, 24576000, 0);

        coeff = get_coeff(es7243->sysclk, params_rate(params));
        if (coeff < 0) {
            coeff = get_coeff(es7243->sysclk / 2, params_rate(params));
            mclkdiv |= 0x20;
        }
        if (coeff < 0) {
            coeff = get_coeff(es7243->sysclk / 3, params_rate(params));
            mclkdiv &= 0x9f;
            mclkdiv |= 0x40;
        }
        if (coeff < 0) {
            coeff = get_coeff(es7243->sysclk / 4, params_rate(params));
            mclkdiv &= 0x9f;
            mclkdiv |= 0x60;
        }
        if (coeff < 0) {
            dev_err(codec->dev,
                ...
                params_rate(params), es7243->sysclk);
            return coeff;
        }
        ...
        /* set iface & srate*/
        es7243_update_bits(ES7243_SDPFMT_REG01, 0x1c, adciface, i2c_clt[i]);
        es7243_update_bits(ES7243_MODECFG_REG00, 0x60, mclkdiv, i2c_clt[i]);
        ...
        if (coeff >= 0) {
            ...
            es7243_update_bits(ES7243_STMOSR_REG0D, 0x3f, osrate, i2c_clt[i]);
            es7243_update_bits(ES7243_BCKDIV_REG03, 0x3f, bclkdiv, i2c_clt[i]);
            es7243_update_bits(ES7243_CLKDIV_REG04, 0x0f, adcdiv, i2c_clt[i]);
            es7243_update_bits(ES7243_LRCDIV_REG02, 0xff, adclrckdiv, i2c_clt[i]);
            es7243_update_bits(ES7243_MODECFG_REG00, 0x0c, speedmode, i2c_clt[i]);

            pr_debug("osrate %x\n", osrate);
            pr_debug("bclkdiv %x\n", bclkdiv);
            pr_debug("adcdiv %x\n", adcdiv);
            pr_debug("adclrckdiv %x\n", adclrckdiv);
            pr_debug("speedmode %x\n", speedmode);
        }
    }
    return 0;
}
...
static struct snd_soc_dai_driver es7243_dai[] = {
#if ES7243_CHANNELS_MAX > 0
    {
        ...
        .symmetric_rates = 1, //disable symmetric rates check because es8388 use different rate to playback
#endif
    },
#endif
...
static int es7243_probe(struct snd_soc_codec *codec)
{
    ...
    char *str;
    int idx;
    micarray_channels = ES7243_CHANNELS_MAX;
    pr_debug("begin->>>>>>! %d\n", es7243_codec_num);
    tron_codec[es7243_codec_num++] = codec;
    if((str = strstr(arcs_cmdline, "mic_gain_ctrl=")))
    {
        str += 14;
        printk("es7243_probe set mic gain to idx=%c%c%c%c\n", str[0], str[1], str[2], str[3]);
        for(idx = 0; idx < 4; idx++)
        {
            gain_ctrl_idx[idx] = str[idx] - '0';
            if(gain_ctrl_idx[idx] > (sizeof(es7243_gain_sel_table) - 1))
                gain_ctrl_idx[idx] = DEFAULT_GAIN_IDX;
        }
    }
    else
    {
        printk("es7243_probe set mic gain to default\n");
        for(idx = 0; idx < 4; idx++)
            gain_ctrl_idx[idx] = DEFAULT_GAIN_IDX;
    }
...
static unsigned int es7243_codec_read(struct snd_soc_codec *codec, unsigned int reg)
{
    u8 val_r;
...
static int es7243_codec_write(struct snd_soc_codec *codec, unsigned int reg, unsigned int value)
{
...
static ssize_t es7243_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    pr_debug("eg read star addres=0x06,count 0x10:echo 0610 >es7243\n");
    pr_debug("eg write star addres=0x90,value=0x3c,count=4:echo 4903c >es7243\n");
                   ...
                   const struct i2c_device_id *i2c_id)
{
    ...
    if (i2c_id->driver_data < (ES7243_CHANNELS_MAX)/2) {
        ...
        pr_debug("i2c_id->driver_data %ld\n", i2c_id->driver_data);
        es7243->regmap = devm_regmap_init_i2c(i2c, &es7243_regmap_config);
        ...
        ret = snd_soc_register_codec(&i2c->dev,
                &soc_codec_dev_es7243, &es7243_dai[i2c_id->driver_data], 1);
        if (ret < 0) {
            pr_debug("ret %d\n", ret);
            kfree(es7243);
            return ret;
...
static int es7243_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
    ...
    pr_debug("client->addr %x", client->addr);
...
// comment this, because it will register in purin-external-card.c
//#if ES7243_CHANNELS_MAX > 0
//    0x10,
//#endif
#if defined (CONFIG_INPUT_MIC_HW4_REF_HW1) || defined(CONFIG_INPUT_MIC_HW4_REF_HW2) ||defined( CONFIG_INPUT_MIC_HW3_REF_HW2)
