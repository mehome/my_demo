...
#ifdef JFFS2_HANDLE_SECURE
#include "../arch/mips/panther/wlan/include/chip.h"
#include "../arch/mips/panther/mtd/erased_page.h"
#endif
...
#if defined(JFFS2_HANDLE_SECURE) && defined(CONFIG_MTD_PDMA_SPI_FLASH)
extern int otp_parse_config(int shift_val);
uint32_t get_secure_data(u32 offset)
{   
	uint32_t secure_data = 0xffffffff;
	u32 ofs = offset % 16;

    if (offset%0x800 < 16)
	{
		secure_data = (u32)__erased_page_data_first[ofs] << 24 | (u32)__erased_page_data_first[ofs + 1] << 16 |
						(u32)__erased_page_data_first[ofs + 2] << 8 | (u32)__erased_page_data_first[ofs + 3];
	}
	else
	{
		secure_data = (u32)__erased_page_data_next[ofs] << 24 | (u32)__erased_page_data_next[ofs + 1] << 16 |
						(u32)__erased_page_data_next[ofs + 2] << 8 | (u32)__erased_page_data_next[ofs + 3];
	}
...
static int jffs2_scan_eraseblock (struct jffs2_sb_info *c, struct jffs2_eraseblock *jeb,
	...
	uint32_t ofs, prevofs, max_ofs = 0;
...
full_scan:
    ...
    {
    	for (ofs = 0; ofs < EMPTY_SCAN_SIZE(c->sector_size); ofs += 4)
        {
			secure_data = get_secure_data(ofs); 

    		if (*(uint32_t *)(&buf[ofs]) != secure_data)
            {
                break;
            }
        }
...
scan_more:
	while(ofs < jeb->offset + c->sector_size) {
		...
		if (*(uint32_t *)(&buf[ofs-buf_ofs]) == secure_data) {
			uint32_t inbuf_ofs;
...
#endif
				if (unlikely(*(uint32_t *)(&buf[inbuf_ofs]) != secure_data)) {
