...
extern uint32_t get_secure_data(u32 offset);
		...
		*bad_offset = ofs;
		...
		for (i=0; i<readlen; i += sizeof(unsigned long)) {
            ...
            if (otp_parse_config(OTP_ENABLE_SECURE_SHIFT)) {     
                u32 temp = *(u32*)UNCACHED_ADDR(ebuf + i);
                u32 secure_data = get_secure_data(i + ofs);
                if (*datum != secure_data && temp != secure_data) {
                    *bad_offset += i;
					printk(KERN_WARNING "temp 0x%x at secure_data 0x%08x\n", temp, secure_data);
