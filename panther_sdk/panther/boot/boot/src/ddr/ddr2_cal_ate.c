...
#ifdef CONFIG_ATE
#define RESULT1  0xBF0048FC //DDR
void ddr_ate_chkrng(void)
{
    u32 addr, j, k, st, se;
    u8 flag=0;

    addr_write(RESULT1, 0);
    u32 data = 0xaaaa5555;
    u32 rd_data = 0xaaaa5555;
    for(k = 0x000; k<0x7ff; k=k+1)
    {
        /* sweep cpu[26:16]= 0/1 */
        st = (k<<16)  | 0x80000000;
        se =  st | 0xFFFF ;
        for(addr = st; addr< se; addr+=4)
        {
            /* sweep cpu[15:0]={A[1:0], bank, col} */
            addr_write(addr, data);
            addr_read(addr, &rd_data);
            if(data != rd_data)
            {
                flag = flag | 1;
            }
...
static void ddr_test(void)
{
    ...
    */
    ...
    addr_read(RESULT1, &val);
    addr_write(RESULT1, dtmp<<11|val);
}
#endif
...
#ifdef CONFIG_ATE
    /* pinmux select gpio id 20 function 3 */
    REG_UPDATE32(PMU_GPIO_FUNC_16_23, (0x3 << 16), (0xf << 16));

    /* change gpio20 to input mode */
    PMUREG_UPDATE32(GPIO_FUNC_EN, 0x100000, 0x100000);
    PMUREG_UPDATE32(GPIO_OEN, 0x100000, 0x100000);

    /* if gpio20 pull high, do ddr_test only */
    if(PMUREG(GPVAL) & 0x100000)
    {
        panther_putc('H');
        ddr_test();
    }
