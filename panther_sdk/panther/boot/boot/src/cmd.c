...
int cmd_go(int argc, char *argv[])
{
    ...
    if (argc > 0)
    {
        ...
        {
            img_type = load_app_image((u32 *)&ih, bootvars.ci_offset, 0);
        ...
        else if ((!strcmp(argv[0], boot_mode_[BOOT_MODE_BUF]))&&(!is_enable_secure_boot))
        {
            ...
            img_type = load_app_image((u32 *)&ih, bootvars.ci_offset, load_loc);
            if (img_type == UNKNOWN_IMAGE_TYPE)
            {
                goto fail;
            }
        ...
        {
            img_type = load_app_image((u32 *)&ih, bootvars.recovery_offset, 0);
        ...
        {
            img_type = load_app_image((u32 *)&ih, bootvars.second_img_offset, 0);
...
int cmd_info(int argc, char *argv[])
{
    ...
    if (argc > 0)
    {
        if (!strcmp(argv[0], "md5"))
            display_app_image_info((u32 *)&ih, bootvars.ci_offset, DISPLAY_APP_INFO_MD5_ONLY);
    }
    else
    {
        display_app_image_info((u32 *)&ih, bootvars.ci_offset, 0);
    }

    return ERR_OK;
}

cmdt cmdt_info __attribute__ ((section("cmdt"))) =
{
"info", cmd_info, "info <md5>; show app image info.\n"};
...
#ifdef CONFIG_EXAM_FIRM_BEFORE_PROGRAM
    ...
    switch (option)
    {
        ...
        case 'a':              // program app
            ...
            if (!memcmp((void *)&ci_magic, (void *)BOOT_BURN_BASE, 4))
            {
                ...
                unsigned char *p_hdr = (unsigned char *)BOOT_BURN_BASE;

                kernel_size = convert_byte_to_size((p_hdr + 0x10), 8);
                root_size = convert_byte_to_size((p_hdr + 0x18), 8);
#if 0
                // check kernel and root separately
                md5_init();
                md5_append((md5_byte_t *)(BOOT_BURN_BASE + 0x20000), kernel_size);
                md5_finish(result);

                rc = verify_md5((p_hdr + 0x40), result);
                if (rc < 0)
                {
                    printf("Invalid Combined-Image !!, kernel corrupt\n");
                    goto fail;
                }

                md5_init();
                md5_append((md5_byte_t *)(BOOT_BURN_BASE + 0x20000 + kernel_size), root_size);
                md5_finish(result);

                rc = verify_md5((p_hdr + 0x60), result);
                if (rc < 0)
                {
                    printf("Invalid Combined-Image !!, root corrupt\n");
                    goto fail;
                }
#else
                // check whole image at one time: kernel + root
                md5_init();
                md5_append((md5_byte_t *)(BOOT_BURN_BASE + 0x20000), kernel_size + root_size);
                md5_finish(result);

                rc = verify_md5((p_hdr + 0x20), result);
                if (rc < 0)
                {
                    printf("Invalid Combined-Image !!\n");
                    goto fail;
                }
#endif
            }
            ...
            {
                // CAPP image header or others
                rc = verify_image(BOOT_BURN_BASE);
                if (rc < 0)
                {
                    printf("Invalid Image !!\n");
                    goto fail;
            ...
            if (is_ubi_image((unsigned char *) src))
            {
                flash_erase_appdata();
