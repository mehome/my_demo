...
#ifdef CONFIG_ATE
/* otp_ate_read() - check all bits in otp */
/*             case 1 - if all zero then return 0 */
/*             case 2 - if only last bit equal 1 then return 1 */
/*        other cases - return negative number */
int otp_ate_read(void)
{
    int idx, ret;
    u8 otp_data_t[OTP_BYTE_SIZE];

    if(0 >= (ret = efuse_read(otp_data_t, 0, OTP_BYTE_SIZE)))
    {
        return OTP_READ_FAILED;
    }
    for(idx = 0; idx < OTP_BYTE_SIZE; idx++)
    {
        /* test fail, return non-zero */
        if(otp_data_t[idx] != OTP_EMPTY_VALUE)
        {
            /* HIT case 1 */
            if((idx == (OTP_BYTE_SIZE - 1)) && (otp_data_t[idx] == 0x1))
                return 1;
            else
                return -1;
        }
...
/* otp_ate_write() - check all bits in otp then write otp last bit to 1 */
/*             Success - return 0 */
/*                Fail - return negative number */
int otp_ate_write(void)
{
    int ret;
    u8 otp_data_t = 0x1;
    if(0 != otp_ate_read())
    ...
    if(0 >= (ret = efuse_write(&otp_data_t, OTP_BYTE_SIZE - 1, 1)))
    {
        return OTP_WRITE_FAILED;
    }
...
int anyboot_cmd(int argc, char *argv[])
{
    if (argc==0)
    {
        printf("Anyboot is ");
        if ((0x01 << OTP_ANYBOOT_ENABLE_SHIFT) & otp_config)
        {
            if (0==((0x01 << OTP_ANYBOOT_DISABLE_SHIFT) & otp_config))
                printf("enabled.\n");
            else
                printf("permanently disabled.\n");
        }
        else
        {
            printf("disabled.\n");
    ...
    {
        otp_write_config((0x01 << OTP_ANYBOOT_ENABLE_SHIFT));
        return 0;
    }
...
cmdt cmdt_anyboot __attribute__ ((section("cmdt"))) =
{
"anyboot", anyboot_cmd, "Usage: anyboot <EnAbLe>\n"};
