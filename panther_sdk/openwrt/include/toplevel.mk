...
%::
	...
	@( \
		...
		if ./scripts/kconfig.pl '>' .config tmp/.config | grep -q CONFIG; then \
			printf "$(_R)ERROR: your configuration is out of sync. Please run make menuconfig, oldconfig or defconfig!$(_N)\n" >&2; \
			false; \
		fi \
	)
	@( \
		cp -f .config tmp/mon_config.h; \
		sed -i -e '/^#/d' -e 's/=.*/ 1/g' -e 's/-/_/g' -e 's/\./_/g' -e '/^$$/d' tmp/mon_config.h; \
		sed -i -e 's/^/#define /g' tmp/mon_config.h; \
	)
