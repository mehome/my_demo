...
PKG_BUILD_DIR_SRC ?= $(BUILD_DIR)/$(PKG_NAME)$(if $(PKG_VERSION),-$(PKG_VERSION))
...
ifdef PRIVATE
PREBUILT_IMAGE=$(wildcard $(PREBUILT_DIR)/$(PKG_NAME)-$(PKG_VERSION).img)
ifdef USE_SRC_DIR
SRC_DIR_EXIST=$(wildcard $(CURDIR)/src)
endif
ifdef USE_SOURCE_DIR
SRC_DIR_EXIST=$(wildcard $(CURDIR)/src)
endif
ifeq ($(SRC_DIR_EXIST),)
ifneq ($(PREBUILT_IMAGE),)
USE_PREBUILT_IMAGE:=1
CMAKE_SOURCE_DIR=$(PKG_BUILD_DIR)
else
USE_SOURCE_DIR=$(CURDIR)/src_public
endif
BUILT_PREBUILT_IMAGE=0
endif
else
BUILT_PREBUILT_IMAGE=1
endif
...
ifdef USE_PREBUILT_IMAGE
  define Build/Prepare/Default
	...
	(cd $(PKG_BUILD_DIR); tar zxf $(PREBUILT_IMAGE))
	touch $(PKG_BUILD_DIR)/configure
	chmod 755 $(PKG_BUILD_DIR)/configure
	echo "all:" >  $(PKG_BUILD_DIR)/Makefile
	echo "install (TARGETS )" > $(PKG_BUILD_DIR)/CMakeLists.txt
...
ifdef USE_SOURCE_DIR
ifdef SEPERATE_SOURCE_BUILD_DIR
  define Build/Prepare/Default
	rm -rf $(PKG_BUILD_DIR_SRC)
	$(if $(wildcard $(USE_SOURCE_DIR_SRC)/*),,@echo "Error: USE_SOURCE_DIR=$(USE_SOURCE_DIR_SRC) path not found"; false)
	ln -snf $(USE_SOURCE_DIR) $(PKG_BUILD_DIR_SRC)
	touch $(PKG_BUILD_DIR_SRC)/.source_dir
  endef
else
  define Build/Prepare/Default
	rm -rf $(PKG_BUILD_DIR)
...
ifdef USE_SRC_DIR
define TSYNC
	-find $(1) -exec touch -t `date +%Y%m%d%H%M.%S` {} +
	echo -e "$(PKG_NAME)_DST=$(1)\n$(PKG_NAME)_SRC=$(CURDIR)" >> $(TMP_DIR)/.build.log
endef
  define Build/Prepare/Default
	rm -rf $(PKG_BUILD_DIR)
	mkdir -p $(PKG_BUILD_DIR)
	$(if $(wildcard ./src/*),,@echo "Error: src path not found"; false)
...
define Build/DefaultTargets
  ...
  $(if $(USE_SRC_DIR)$(USE_SOURCE_DIR)$(USE_GIT_TREE),,$(if $(strip $(PKG_SOURCE_URL)),$(call Download,default)))
