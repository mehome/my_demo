...
$(FILELIST): $(OVERRIDELIST)
	...
	$(call FIND_L, $(SCAN_DIR)) $(SCAN_EXTRA) -mindepth 1 $(if $(SCAN_DEPTH),-maxdepth $(SCAN_DEPTH)) -not -path "package/legacy/*" -name Makefile | xargs grep -aHE 'call $(GREP_STRING)' | sed -e 's#^$(SCAN_DIR)/##' -e 's#/Makefile:.*##' | uniq >> $(TMP_DIR)/info/.files-$(SCAN_TARGET)-temp
	$(call FIND_L, package/legacy) $(SCAN_EXTRA) -mindepth 1 $(if $(SCAN_DEPTH),-maxdepth $(SCAN_DEPTH)) -name Makefile | xargs grep -aHE 'call $(GREP_STRING)' | sed -e 's#^$(SCAN_DIR)/##' -e 's#/Makefile:.*##' | uniq >> $(TMP_DIR)/info/.files-$(SCAN_TARGET)-temp
	cat $(TMP_DIR)/info/.files-$(SCAN_TARGET)-temp | awk -v of=$(OVERRIDELIST) -f include/scan.awk > $@
