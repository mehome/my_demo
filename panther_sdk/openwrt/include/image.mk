...
#
include $(INCLUDE_DIR)/project.mk

VERSION_VAL		= $(shell date "+%Y%m%d%H%M%S")
VERSION_PATH	= $(TARGET_DIR)/etc/sw_build
SRC_PATH		= $(BIN_DIR)/combined-uzImage-ubi-overlay
ifeq ($(CONFIG_PROJECT_BEDLAMP_V1),y)
    DST_PATH		= $(BIN_DIR)/FW_panther_$(VERSION_VAL)_bedlamp.img
else ifeq ($(CONFIG_PROJECT_K2_V1),y)
    DST_PATH                = $(BIN_DIR)/FW_panther_$(VERSION_VAL)_K2.img
else
    DST_PATH                = $(BIN_DIR)/FW_panther_$(VERSION_VAL).img
endif
...
ifneq ($(CONFIG_BIG_ENDIAN),)
  JFFS2OPTS     :=  --big-endian --squash-uids -v -n
else
  JFFS2OPTS     :=  --little-endian --squash-uids -v -n
endif
...
ZLIB_OPTIONS :=
LZMA_XZ_OPTIONS :=
LZO_OPTIONS := -Xcompression-level 9
LZ4_OPTIONS := -Xhc
ZSTD_OPTIONS := -Xcompression-level 22
ifeq ($(CONFIG_SQUASHFS_ZLIB),y)
  SQUASHFSCOMP := gzip $(ZLIB_OPTIONS)
endif
ifeq ($(CONFIG_SQUASHFS_LZMA),y)
  SQUASHFSCOMP := lzma $(LZMA_XZ_OPTIONS)
endif
ifeq ($(CONFIG_SQUASHFS_LZO),y)
  SQUASHFSCOMP := lzo $(LZO_OPTIONS)
endif
...
fs-subtypes-$(CONFIG_TARGET_ROOTFS_JFFS2_NAND) += $(addsuffix -raw,$(addprefix jffs2-nand-,$(NAND_BLOCKSIZE)))
...
define Image/mkfs/squashfs
	$(STAGING_DIR_HOST)/bin/mksquashfs4 $(TARGET_DIR) $(KDIR)/root.squashfs -sort $(TOPDIR)/target/linux/panther/image/sort_file -nopad -noappend -root-owned -comp $(SQUASHFSCOMP) $(SQUASHFSOPT) -processors $(if $(CONFIG_PKG_BUILD_JOBS),$(CONFIG_PKG_BUILD_JOBS),1)
endef
...
ifneq ($(CONFIG_TARGET_ROOTFS_UBIFS),)
	...
	$(if $(wildcard ./ubinize.cfg),$(call Image/Build,ubi-overlay))
    endef
endif
...
define Image/mkfs/prepare/default
	$(TOPDIR)/scripts/get-git-version.sh $(TOPDIR)/package
	rm -f $(TARGET_DIR)/etc/version
	cp -f .gitversion $(TARGET_DIR)/etc/version
	cat $(LINUX_DIR)/.gitversion >> $(TARGET_DIR)/etc/version
	echo -n $(VERSION_VAL) > $(VERSION_PATH)
