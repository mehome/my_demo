...
ifeq (,$(wildcard $(STAGING_DIR_HOST)/bin/sed))
$(error Please exec ./install_host_tools.sh to install host tools!)
endif
KERNEL_CONFIG_FILE=${shell $(STAGING_DIR_HOST)/bin/grep CONFIG_EXTERNAL_KERNEL_CONFIG_FILE $(TOPDIR)/.config | $(STAGING_DIR_HOST)/bin/sed -e "s/CONFIG_EXTERNAL_KERNEL_CONFIG_FILE=//g" | $(STAGING_DIR_HOST)/bin/sed -e "s/\"//g" }
ifeq ($(strip $(KERNEL_CONFIG_FILE)),)
LINUX_TARGET_CONFIG = $(call find_kernel_config,$(PLATFORM_DIR))
else
GENERIC_LINUX_CONFIG = $(call find_kernel_config,$(PLATFORM_DIR))
LINUX_TARGET_CONFIG := $(PLATFORM_DIR)/configs/kernel/$(KERNEL_CONFIG_FILE)
endif
