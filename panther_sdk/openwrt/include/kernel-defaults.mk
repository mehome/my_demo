...
ifeq ($(strip $(CONFIG_EXTERNAL_KERNEL_TREE)),"")
  ...
  define Kernel/Prepare/Default
	...
	#cp -al $(CONFIG_EXTERNAL_KERNEL_TREE) $(LINUX_DIR)
  endef
endif
...
# copy CONFIG_KERNEL_* settings over to .config.target
	...
	cp -f $(LINUX_DIR)/.config $(TMP_DIR)/mon_kconfig.h
	sed -i -e '/^#/d' -e 's/=.*/ 1/g' -e 's/-/_/g' -e 's/\./_/g' -e '/^$$$$/d' $(TMP_DIR)/mon_kconfig.h
	sed -i -e 's/^/#define /g' $(TMP_DIR)/mon_kconfig.h
	mkdir -p $(STAGING_DIR)/usr/include
	cp -f $(TMP_DIR)/mon_config.h $(STAGING_DIR)/usr/include
	cp -f $(TMP_DIR)/mon_kconfig.h $(STAGING_DIR)/usr/include
endef
