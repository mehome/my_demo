...
static int send_package(char *buf, char len)
{
	int sockfd=-1;
	...
	if ((numbytes=sendto(sockfd, buf, len, 0,(struct sockaddr *)&broadcastAddr, sizeof(broadcastAddr))) == -1) 
    {
		perror("sendto");
		ret = -1;
		goto done;
...
static void mm_startlandiscover(void)
{
	...
	os_timer_arm(&time_serv, 5000, 1);//5s
}
#endif
...
void recv_thread(void) 
{
	...
	//inet_pton(AF_INET, "10.10.10.255",  &addrto.sin_addr);
	char buf[32]= {0};
	//int ret = get_local_ip("br1", buf);
	//if(ret == 0) {
	//	inet_pton(AF_INET, buf, &addrto.sin_addr);
	//} else {
		inet_pton(AF_INET, "255.255.255.255", &addrto.sin_addr);
	//}
...
static void airkiss_apply(void)
{
    ...
    broadcastAddr.sin_family = AF_INET;
    //inet_pton(AF_INET, "255.255.255.255", &broadcastAddr.sin_addr);
    //inet_pton(AF_INET, "192.168.23.255", &broadcastAddr.sin_addr);
    //inet_pton(AF_INET, "10.10.10.255", &broadcastAddr.sin_addr);
    char tmp[32]= {0};
	//int ret = get_local_ip("br1", tmp);
	//if(ret == 0) {
	//	inet_pton(AF_INET, tmp, &broadcastAddr.sin_addr);
	//} else {
		inet_pton(AF_INET, "255.255.255.255", &broadcastAddr.sin_addr);
	//}
...
int main(int argc, char *argv[])
{
	...
	strcat(g_device_id,deviceId);
#endif	
	cprintf("%s:%d g_device_id:\"%s\" appid:\"%s\" , discoverType:%d\n\n", __func__, __LINE__,g_device_id,appid, discoverType);
...
#else
static void airkiss_discover_thread(void)
{
	while(1) {
		sleep(2);
		time_callback();
...
int main(int argc, char *argv[])
{
	...
	pthread_attr_setstacksize(&tattr, PTHREAD_STACK_MIN);
	//pthread_attr_setdetachstate(&tattr, PTHREAD_CREATE_DETACHED);
	ret = pthread_create(&airkiss_rcv_thread_id, &tattr, airkiss_discover_thread, NULL);
	pthread_attr_destroy(&tattr);
	...
	daemon(0, debug);
	pthread_join(airkiss_rcv_thread_id, NULL);
done:
	PidUnLock();
