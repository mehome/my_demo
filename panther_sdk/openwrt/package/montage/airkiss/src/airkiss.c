...
#else
#define airkiss_output(fmt, args...) do { \
        FILE *fp = fopen("/dev/console", "w"); \
        if (fp) { \
                fprintf(fp, fmt , ## args); \
                fclose(fp); \
...
void set_ssid_pwd_and_config(char *ssid, char *pass)
{
    char cmd[512] = {0};
...
static void airkiss_finish(struct mon_airkiss *monair)
{
    ...
    airkiss_get_result(akcontex, &result);//获取airkiss解码的结果放入result

    monair->is_wifi_scan = true;

    wifi_promiscuous_enable(false);

    airkiss_output("airkiss:ssid = \033[32m\"%s\"\033[0m, pwd = \033[32m\"%s\"\033[0m, ssid_length = %d, "
            "pwd_length = %d, random = 0x%02x\n",
            result.ssid, result.pwd, result.ssid_length,
            result.pwd_length, result.random);
    //收到了配网信息，开始联网，超时时间30秒
//    eval_nowait("aplay","-D","common","/tmp/res/receive_network_info.wav");//收到网络信息
	exec_cmd("aplay -D common /tmp/res/receive_network_info.wav");
    ...
    cdb_set("$op_work_mode", "9");
    eval_nowait("mtc","wlhup");
#else
	    set_ssid_pwd_and_config(result.ssid, (result.pwd_length != 0) ? result.pwd : NULL);
#endif
#else
...
static void airkiss_handler(struct mon_airkiss *monair)
{
    ...
    while(!stop) {
        ...
        len = recv(monair->sockfd, g_buffer, sizeof g_buffer, 0);	//接收手机app发送的数据包
        ...
        if(buf_header && (t_len > 24))
        {
            ...
            else
            {
                ...
                if (ret == AIRKISS_STATUS_CHANNEL_LOCKED)
                {
                    //正在进行airkiss配网，把声波配网线程停掉
                    cdb_set_int("$sinvoice_state",0);
...
static void airkiss_apply(void)
{
    ...
    cdb_set_int("$airkiss_start",0); //本次配网结束
    alarm(0); //取消定时器
...
/* BEGIN: Added by Frog, 2018-9-5 10:55:9*/
void airkiss_sigalrm_fn(int sig)  
{
    airkiss_timeout = 1;
    exec_cmd("mpg123.sh /tmp/res/airkiss_timeout.mp3");
    cdb_set_int("$airkiss_start",0);
}
//charset:utf-8
/*  airkiss配网逻辑如下：
 *  1.设备端依次扫描监听各个信道，收到指定格式的配网包后，认为当前是用的airkiss配网
 *  2.把声波配网程序kill掉，并将其状态位清理干净
 *  3.设备端收到完整的配网 SSID和密码后，设置airkiss配网状态为2，并开始联网
 *  4.联网成功后，发送广播包，告诉手机端已配网成功，手机端切换到设备发现页面
 *  5.设置airkiss配网状态为3,配网开始标志设置为0
*/

/* END:   Added by Frog, 2018-9-5  10:55:13*/
...
int main(int argc, char *argv[])
{
    ...
    else if (!strcmp("start", argv[1]))			//按键配网，执行这个函数
    {
        int debug = 0;
        unsigned int state;
        cdb_set_int("$airkiss_start",1); //airkiss开始配网了
        state = cdb_get_int("$airkiss_state", AIRKISS_STANDBY);
        ...
        signal(SIGALRM, airkiss_sigalrm_fn);
        ...
        ret = start_airkiss(monair);//开始初始化airkiss库的东西
        if (ret)
        {
        ...
        mon_srv_stop();
        wifi_set_channel(monair, true);	//wifi设置信道
        wifi_promiscuous_enable(true);

        monair->sockfd = open_monitor(AK_MONITOR);	//获取socket套接字

        /*Recv Thread */
        if (pthread_create((pthread_t *) & (monair->rcv_thread_id), NULL,
        ...
        while (!monair->is_airkiss_complete)
        {
            ...
            if (time(NULL) > stop_time)
            {
                ...
                exec_cmd("mtc commit");
                //始终没有收到airkiss空气包，超时
                exec_cmd("aplay -D common /tmp/res/airkiss_timeout.wav");//联网未成功，请再试一次
                
#if defined (CONFIG_PROJECT_K2_V1) || defined (CONFIG_PROJECT_DOSS_1MIC_V1)
				exec_cmd("uartdfifo.sh StartConnectedFAL");
#endif

                cdb_set_int("$airkiss_start",0);	//配网结束时候,airkiss_start:0
                cdb_set_int("$airkiss_state", AIRKISS_TIMEOUT);
                cprintf("\033[41;32mAirKiss Timeout, To AP MODE\033[0m\n");
                break;
            }
        }
        close(fd);
