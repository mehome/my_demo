...
var wdk = (function() {
	...
	var COMMIT_STATE_CMD = CGIFILE + '?cmd=commit state';
	...
	function request_submit(action) {
		var cmds = '';
		var cnum = 0;
		...
		var post = "cnum=" + cnum + "&cmd=" + encodeURI(cmds).replace(/&/g, '%26');

		httpReq(CGIFILE, post);
	...
	function showResultBG() {
		...
		document.getElementById("resultbg").style.visibility = "visible";
	}
	...
	function hideResultBG() {
		...
		document.getElementById("resultbg").style.visibility = "hidden";
	}
	...
	function msg(key) {
		...
		try {
			if (lid.match(/!ERR/mg) || lid.match(/^en$/mg))
				...
				if (!m) {
					/*WSIM_BEGIN*/
					console.log('MSG:[' + key + '] not mapping, check file:[' + _msg_file + ']');
					/*WSIM_END*/
					return key;
				}
				return m;
	...
	function redirect_restore(td, rd) {
		...
		document.getElementById("titleupwarng").innerHTML = msg("Rebooting! Please wait ") + td + msg(" seconds");
		if (td == 0)
