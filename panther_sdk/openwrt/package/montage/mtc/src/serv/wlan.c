...
#if defined(__PANTHER__)
#define WIFI_NOT_CONNECTED_SOUND_FILE "/tmp/res/speaker_not_connected.wav"
static void play_wifi_not_connected_sound(void)
{
    struct stat buf;
    if(stat(WIFI_NOT_CONNECTED_SOUND_FILE, &buf) == 0)
    {
        system("/usr/bin/wavplayer " WIFI_NOT_CONNECTED_SOUND_FILE " &");
    }
}
#endif
        ...
        if (cdb_get_int("$wanif_state", Wanif_State_DISCONNECTED) != Wanif_State_CONNECTED) {
            if (cdb_get_int("$op_work_mode", opMode_ap) != opMode_ap) {
                if(cdb_get_int("$network_caller", 0) == 0)
                {
                    ...
                    play_wifi_not_connected_sound();		//音响未联网，请连接网络
