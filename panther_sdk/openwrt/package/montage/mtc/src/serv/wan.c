...
int mtdm_wan_ip_up(BootState boot, montage_proc_t *ptbl, int tblcnt)
{
    ...
    if(caller_status == 0)
    {
        ...
        if(f_exists("/usr/bin/turingIot"))   //如果没有iot模块就不再执行相应的动作
        {
            ...
            cdb_set_int("$iot_nocheck", 0);
        }
    }
    else if(caller_status == 1)
    {
        exec_cmd("aplay -D common /tmp/res/connected.wav");//联网成功  
#if defined (CONFIG_PROJECT_K2_V1) || defined (CONFIG_PROJECT_DOSS_1MIC_V1)
		exec_cmd("uartdfifo.sh StartConnectedOK");
#endif
...
#ifdef USE_TURING
        //airkiss配网成功
        if(cdb_get_int("$airkiss_start", 0))
        {
            exec_cmd("airkiss apply &");
        }

        //声波配网成功
        if(cdb_get_int("$sinvoice_start",0) == 1)
        {
            OnWriteMsg("/tmp/UNIX.turing","SinvoiceSuccess");
        }
#endif
...
#if 1
	exec_cmd("/usr/bin/check-version");		//检查版本
	if(f_exists("/usr/bin/ota"))
		exec_cmd("/usr/bin/ota &");
#else
	char val[1] = {0};
