...
void mount_tf_and_usb(char *dev_type,char *action,char *dev_name)
{
    ...
    sprintf(mntdev, "/dev/%s", dev_name);
    sprintf(mntpath, "/media/%s", dev_name);

    sysprintf("mount %s to %s\n",mntdev,mntpath);
    ...
    if (!strcmp(action, "add")) 
    {
        ...
        sysprintf("mount to %s\n",mntpath);
        if(dev_name[0] == 's')
        {
            cdb_set_int("$usb_status", 1);
            cdb_set("$usb_mount_point", dev_name);
        }
        else if(dev_name[0] == 'm')
        {
            cdb_set_int("$tf_status", 1);
            cdb_set("$tf_mount_point", dev_name);
        }
    }
}
...
void umount_tf_and_usb(char *dev_type,char *action,char *dev_name)
{
    char mntdev[128] = {0};
    char mntpath[128] = {0};
    char *remove_dev = NULL;
    char current_mode[8] = {0};
    
    sprintf(mntdev, "/dev/%s", dev_name);
    sprintf(mntpath, "/media/%s", dev_name);

    cdb_get_str("$play_mode",current_mode,8,"wifi");
    
    if(dev_name[0] == 's')
    {
        cdb_set_int("$usb_status", 0);
        cdb_set("$usb_mount_point", "");
        remove_dev = "usb";
    }
    else if(dev_name[0] == 'm')
    {
        cdb_set_int("$tf_status", 0);
        cdb_set("$tf_mount_point", "");
        remove_dev = "tf";
    }
    ...
    if( _isFileExist (mntpath) ) 
    {
        usleep(1000000);
        if(!strcmp(current_mode,remove_dev)){
            eval("mpc", "clear");
        }
        if( umount2(mntpath, MNT_FORCE) != 0 ) {
            eval("umount", "-f", mntpath);
...
*/
 int search_and_check_firmware(char *path)
 {
	...
	cslog("path = [%s].......\n",path);//search_and_check_firmware:path = [/media/mmcblk0p1].......
...
hotplug_block(char *type, char *action, char *name )
{
	...
	sprintf(mntpath, "/media/%s", name);//mmcblk0p1
   ...
   */ 
        ...
        else if(name[0] == 'm' ) 		//tf卡模式
        {
		    sys_auto_upgrade = cdb_get_int("$sys_auto_upgrade", 0); //sys_auto_upgrade:1
			if(sys_auto_upgrade)
			{
			    cslog("upgrade in------------\n");
				ret = search_and_check_firmware(mntpath);
            ...
            {
                cdb_set_int("$tf_status", 1);
                cdb_set("$tf_mount_point", name);//$tf_mount_point='mmcblk0p1'
                return 0;
            }
            eval("mpc", "clear");		//清除之前的播放列表
            usleep(100*1000);
            
            delete_file("/usr/script/playlists/tfPlaylist.m3u");
            delete_file("/usr/script/playlists/tfPlaylistInfoJson.txt");	//如果之前有插入过tf卡，会遗留这两个文件，再次插入需删除
            
            eval("mpc", "update",name);		//更新下/media/mmcblk0p1路径下的内容
            usleep(200*1000);
            eval("uartdfifo.sh", "tfint");	
            usleep(200*1000);
            eval("uartdfifo.sh", "intfmode");
            usleep(200*1000);
            ret = cdb_set_int("$tf_status", 1);	//将tf卡标志设置为1
            ret = cdb_set("$tf_mount_point", name);
            sleep(1);

			if(cdb_get_int("$vtf003_usb_tf",0)){		//没有执行
			...
			exec_cmd(tempbuff);
                ...
                eval("playlist.sh", "tfautoplay");	//tf开始播放
			}
            OnWriteMsg("/tmp/UNIX.turing","UpdateDevice");
        }
		
		save_diskinfo();
	} 
	else if (!strcmp(action, "remove"))			//拔出tf卡
	{
//		goto exit1;
	    //cslog("remove ------------\n");
        char value[10] ={0};
		int usb_status = cdb_get_int("$usb_status",0);	//usb_status:0
		int tf_status = cdb_get_int("$tf_status",0);	//tf_status:1
    	...
    	else if (name[0] == 'm')			//tf模式
    	{
    		if(tf_status != 0)							//执行
    		{
    			...
    			if (strcmp(value, "004") == 0)
    			{
        			eval("mpc", "clear");
        			if(!cdb_get_int("$vtf003_usb_tf",0)){
        			   cdb_set("$playmode", "000");		//如果拔掉tf卡，更改模式
        			}
                }
                ret = cdb_set("$tf_mount_point", "");
                eval("mpc", "update", name );			//更新
                
    			cdb_set("$tf_status", "0");				
				
                delete_file("/usr/script/playlists/tfPlaylist.m3u");
                delete_file("/usr/script/playlists/tfPlaylistInfoJson.txt");
				delete_file("/usr/script/playlists/currentPlaylistInfoJson.txt");	//删除这些tf的列表
				OnWriteMsg("/tmp/UNIX.turing","UpdateDevice");
...
void hotplug_button(char *button, char *action, char *seen)
{
 ...
 int OnWriteMsg(char *path,  char* cmd)
{
	int iRet = -1;
    int iConnect_fd = -1;
	int flags = -1;
	...
	if(iConnect_fd<0)
	{
		printf("\033[1;32;40m %s:%s:%d cannot create communication socket. \033[0m\n",__FILE__, __func__, __LINE__);
		return 1;
	}
	...
	{
		printf("\033[1;32;40m %s:%s:%d connect server failed.. [write:<%s>]\033[0m\n",__FILE__, __func__, __LINE__, cmd);
	}
