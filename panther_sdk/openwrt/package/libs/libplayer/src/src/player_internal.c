...
player_sv_mrl_append (player_t *player, mrl_t *mrl, player_mrl_add_t when)
{
  ...
  if(when == PLAYER_MRL_INSERT_QUEUE)
  {
    /* insert mrl to the first */
    pl_playlist_insert_mrl (player->playlist, mrl);
  }
  else
  {
...
#ifdef QQ_COMPATIBLE
void
player_sv_mrl_select (player_t *player, int idx)
{
  int i;
  pl_log (player, PLAYER_MSG_VERBOSE, MODULE_NAME, __FUNCTION__);
...
#ifdef QQ_COMPATIBLE
int
player_sv_get_time_len (player_t *player)
{
  int res = -1;
  ...
  /* player specific get_time_len() */
  PLAYER_FUNCS_RES (get_time_len, res)
...
void
player_sv_request_stream_read_count (player_t *player)
{
  pl_log (player, PLAYER_MSG_VERBOSE, MODULE_NAME, __FUNCTION__);
  ...
  PLAYER_FUNCS (request_stream_read_count);
}
