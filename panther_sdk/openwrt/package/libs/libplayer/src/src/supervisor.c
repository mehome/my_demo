...
#ifdef QQ_COMPATIBLE
static void
supervisor_player_mrl_select (player_t *player,
                                 pl_unused void *in, pl_unused void *out)
{
  supervisor_data_mrl_t *input = in;
  ...
  player_sv_mrl_select (player, input->value);
}
#endif
...
#ifdef QQ_COMPATIBLE
static void
supervisor_player_get_time_len (player_t *player,
                                pl_unused void *in, void *out)
{
  int *output = out;
  ...
  *output = player_sv_get_time_len (player);
}
#endif
...
static void
supervisor_player_request_stream_read_count(player_t *player)
{
  if (!player)
      return;

  player_sv_request_stream_read_count(player);
}
...
static void (*g_supervisor_funcs[]) (player_t *player, void *in, void *out) = {
...
#ifdef QQ_COMPATIBLE
  [SV_FUNC_PLAYER_MRL_SELECT]            = supervisor_player_mrl_select,
#endif
  /* Player tuning & properties */
  [SV_FUNC_PLAYER_GET_TIME_POS]          = supervisor_player_get_time_pos,
#ifdef QQ_COMPATIBLE
  [SV_FUNC_PLAYER_GET_TIME_LEN]          = supervisor_player_get_time_len,
#endif
  ...
  [SV_FUNC_PLAYER_REQUEST_STREAM_READ_COUNT] = supervisor_player_request_stream_read_count,
  /* Audio related controls */
