...
#ifdef QQ_COMPATIBLE
  [PROPERTY_TIME_LEN]         = ILT ("length",           ITEM_ON ),
#endif
...
thread_fifo (void *arg)
{
  ...
  while (fgets (buffer, FIFO_BUFFER, mplayer->fifo_out))
  {
    ...
    if (strstr (buffer, "EOF code:") == buffer)
    {
      ...
      pthread_mutex_lock (&mplayer->mutex_status);
#ifdef QQ_COMPATIBLE
      if ((mplayer->status == MPLAYER_IS_PLAYING) || (mplayer->status == MPLAYER_IS_IDLE))
#else
...
slave_action (player_t *player, slave_cmd_t cmd, slave_value_t *value, int opt)
{
  ...
  case SLAVE_REQUEST_STREAM_READ_COUNT:
    send_to_slave (player, "%s %s", "pausing_keep_force", command);
...
static char *
mp_resource_get_uri_unknown (const char *protocol,
                           mrl_resource_local_args_t *args)
{
  return strdup (args->location);
}
...
mp_resource_get_uri (mrl_t *mrl)
{
  ...
  case MRL_RESOURCE_UNKNOWN:
    return mp_resource_get_uri_unknown(protocols[mrl->resource], mrl->priv);
...
//    params[pp++] = "8192";
  ...
  default:
  {
    ...
    /* set fifo out to non-block */
#ifndef QQ_COMPATIBLE
    fifo_out_fd = fileno(mplayer->fifo_out);
    fcntl(fifo_out_fd, F_SETFL, (fcntl(fifo_out_fd, F_GETFL) | O_NONBLOCK));
#endif
...
#else
#define MP_SLAVE_CTL_IN  "/tmp/.mplayer.in"
#define MP_SLAVE_CTL_OUT "/tmp/.mplayer.out"
    pthread_attr_t attr;

    mplayer->fifo_in = fopen (MP_SLAVE_CTL_IN, "w");
    mplayer->fifo_out = fopen (MP_SLAVE_CTL_OUT, "r");
    pl_log (player, PLAYER_MSG_INFO, MODULE_NAME, "MPlayer client connected");
...
static void
mplayer_request_stream_read_count (player_t *player)
{
  pl_log (player, PLAYER_MSG_VERBOSE, MODULE_NAME, "get_stream_read_count");

  if (!player)
    return;

  slave_cmd (player, SLAVE_REQUEST_STREAM_READ_COUNT);
}
...
#ifdef QQ_COMPATIBLE
static int
mplayer_get_time_len (player_t *player)
{
  float time_len = 0.0;

  pl_log (player, PLAYER_MSG_VERBOSE, MODULE_NAME, "get_time_len");

  if (!player)
    return -1;

  time_len = slave_get_property_float (player, PROPERTY_TIME_LEN);

  if (time_len < 0.0)
    return -1;

  return (int) (time_len * 1000.0);
}
#endif
...
pl_register_functions_mplayer (void)
{
  ...
  funcs->request_stream_read_count = mplayer_request_stream_read_count;
  funcs->get_time_pos       = mplayer_get_time_pos;
#ifdef QQ_COMPATIBLE
  funcs->get_time_len       = mplayer_get_time_len;
#endif
