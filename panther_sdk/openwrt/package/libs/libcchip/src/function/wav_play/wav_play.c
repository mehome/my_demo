...
#define WAV_PLAY_OUT_DEVICE "common"
...
static
void snd_wav_play_preload(snd_pcm_t *pcm, sndpcmcontainer_t *sndpcm, wavcontainer_t *wav, int fd, off64_t pcm_len)
{
	int ret = 0;
...
int wav_play(const char *filename)
{
	...
	if ((ret = snd_pcm_open(&pcm,WAV_PLAY_OUT_DEVICE, SND_PCM_STREAM_PLAYBACK, 0)) < 0) {
	...
	snd_wav_init(pcm, &pb, &wav, pcm_len);
...
/* use wav_play_preload to avoid underrun, but waste memory */
int wav_play_preload(const char *filename)
{
	...
	snd_wav_init(pcm, &pb, &wav, pcm_len);
	snd_wav_play_preload(pcm, &pb, &wav, fd, pcm_len);
