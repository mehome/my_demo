...
#define DEFAULT_MAX_LEN			 (12)		//经YIN测试12s是录音长度极限，继续加长时间会出现Underrun错误
...
{ 
	...
	if(wav->wav_len > 0){
		if(wav->wav_len > DEFAULT_MAX_LEN){
			duration_time = DEFAULT_MAX_LEN; 
		}else{
			duration_time = wav->wav_len;
		}
	}else{
...
{ 
	...
	if(GetState_StopRecord()){
		SetState_StopRecord(0);
	}
...
//main入口
int WavRecorder(char *filename, unsigned int sec) 
{ 
