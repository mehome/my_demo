...
#if defined(CONFIG_HIDE_SSID_STA_CONNECTED) || defined(CONFIG_MONTAGE_APCLIENT)
static int hostapd_reload(struct wpa_supplicant *wpa_s, struct wpa_bss *bss, int hide_ssid)
#else
...
#endif
{
	...
	int sec_chan = 0;
	int ret = 0;
#ifdef CONFIG_INFORM_DRIVER_PWR_ONOFF
	int mode;
#endif
...
#ifdef CONFIG_HIDE_SSID_STA_CONNECTED
	if (asprintf(&cmd, "UPDATE channel=%d sec_chan=%d hw_mode=%d ignore_broadcast_ssid=%d", 
		     channel, sec_chan, hw_mode, (hide_ssid ? cdb_get_int("$wl_wisp_hide_ssid", 0) : 0)) < 0)
		return -1;
#else
...
#ifdef CONFIG_HIDE_SSID_STA_CONNECTED
#ifdef CONFIG_INFORM_DRIVER_PWR_ONOFF
	mode = cdb_get_int("$op_work_mode", 0);
	if(!hide_ssid)
		system("echo wifi_poweroff_enable=0 > /proc/wla ; hostapd_cli update_beacon");
	else if ((9 == mode))
		system("echo wifi_poweroff_enable=1 > /proc/wla ; hostapd_cli stop_ap");
	else
		wpa_printf(MSG_DEBUG, "\nDo Nothing on mode:%d and sta connected\n",mode);
#endif
#else
#ifdef CONFIG_INFORM_DRIVER_PWR_ONOFF
	system("echo wifi_poweroff_enable=0 > /proc/wla");
#endif
#endif
...
int omnicfg_notify_auth_failure(void);
void omnicfg_notify_no_network_found(void);

int omnicfg_auth_failure;
int omnicfg_no_network_found;
int omnicfg_max_wpa_retry;

#define TOTAL_CHANNELS  14
int channel_freq[14] = { 2412,2417,2422,2427,2432,2437,2442,2447,2452,2457,2462,2467,2472,2484 };
#define WORK_MODE_STA   9

void omnicfg_notify_channel_change(int freq)
{
    int i;
    int op_work_mode;

    op_work_mode=cdb_get_int("$op_work_mode", 0);
    if (op_work_mode != WORK_MODE_STA)
        return ;
    ...
    for(i=0;i<TOTAL_CHANNELS;i++)
    {
        ...
        {
            cdb_set_int("$omi_iw_channel", (i+1));
            system("/usr/bin/killall -SIGUSR2 hostapd");
            return;
        }
    }
}

void omnicfg_wpa_supp_init(void)
{
    omnicfg_max_wpa_retry = cdb_get_int("$omi_wpa_supp_retry", 0);
    cdb_set_int("$omi_wpa_supp_retry", 0);

    omnicfg_auth_failure = 0;
    omnicfg_no_network_found = 0;
}

int omnicfg_max_wpa_retry_get(void)
{
    return omnicfg_max_wpa_retry;
}

int omnicfg_notify_auth_failure(void)
{
    if(omnicfg_max_wpa_retry)
    {
        omnicfg_auth_failure++;

        if(omnicfg_auth_failure>=3)
        {
                cdb_set_int("$omi_wpa_supp_result", 2);         /* WPA auth failure */
                exit(0);
        }
...
void omnicfg_notify_no_network_found(void)
{
    if(omnicfg_max_wpa_retry)
    {
        omnicfg_no_network_found++;

        if(omnicfg_no_network_found>=omnicfg_max_wpa_retry)
        {
                cdb_set_int("$omi_wpa_supp_result", 3);         /* Find AP retry to limit */
                exit(0);
        }
    }
}

void omnicfg_check_connect_state(struct wpa_supplicant *wpa_s, enum wpa_states state)
{
    char val[4];
    ...
    switch (state)
    {
        case WPA_COMPLETED:
            ...
            {
                // (none | wep) : complete_state=1
                snprintf(val, 4, "%d", 1);
                cdb_set("$omi_wpa_supp_state", val);
                cdb_set("$omi_wpa_supp_result", val);
            } else if (wpa_s->wpa_state == WPA_GROUP_HANDSHAKE) {
                // (wpa | wpa2) : complete_state=2
                snprintf(val, 4, "%d", 2);
                cdb_set("$omi_wpa_supp_state", val);
                snprintf(val, 4, "%d", 1);
                cdb_set("$omi_wpa_supp_result", val);
            }
            omnicfg_max_wpa_retry = 0;
            break;
    }
    return ;
}
...
#if defined(CONFIG_OMNICFG)
	omnicfg_check_connect_state(wpa_s, state);
#endif
...
#if defined(CONFIG_HIDE_SSID_STA_CONNECTED)
		if (wpa_s->hostapd)
...
#ifndef CONFIG_MONTAGE_APCLIENT
		if (wpa_s->hostapd)
			hostapd_stop(wpa_s);
#else
		if (wpa_s->hostapd && wpa_s->current_bss)
		{
			if(state == WPA_DISCONNECTED) {
				if(old_state == WPA_DISCONNECTED) {
					hostapd_stop(wpa_s);
				}
				else
				{
...
#if defined(CONFIG_OMNICFG)
	if(omnicfg_notify_auth_failure())
		return;
#endif
