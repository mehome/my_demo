...
extern int omnicfg_max_wpa_retry_get(void);
#endif
static int wpas_select_network_from_last_scan(struct wpa_supplicant *wpa_s,
					      int new_scan, int own_request)
{
...
#if defined(CONFIG_OMNICFG)
		omnicfg_notify_no_network_found();
#endif
#ifdef CONFIG_MESH
		if (wpa_s->ifmsh) {
			wpa_msg(wpa_s, MSG_INFO,
...
#ifdef CONFIG_MONTAGE_WISP_PATCH
			if(wpa_s->conf->scan_interval)
				timeout_sec = wpa_s->conf->scan_interval;
#endif // CONFIG_MONTAGE_WISP_PATCH
...
#if defined(CONFIG_OMNICFG)
			/* Reduce the scan_interval when the omnicfg is retrying to connect AP
			*  omnicfg_max_wpa_retry_get: return the value of omnicfg_max_wpa_retry.
			*  The returned value as following.
			*  0:       original version
			*  else:    omniconfig version
			*/
			if (omnicfg_max_wpa_retry_get())
			{
				timeout_sec = 0;
				timeout_usec = 250000;
			}
