...
#ifdef CONFIG_HIDE_SSID_STA_CONNECTED
		else if ((val = get_option(opt, "ignore_broadcast_ssid="))) {
			if(bss)
				bss->ignore_broadcast_ssid = atoi(val);
		}
#endif
		else
			break;
...
static int hostapd_ctrl_iface_update(struct hostapd_data *hapd, char *txt)
{
	...
	reload_opts = txt;
#else
	strncpy(update_opts, txt, 256);
	reload_opts = update_opts;
#endif
