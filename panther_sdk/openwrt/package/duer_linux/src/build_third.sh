...
compile_nghttp2() {
	...
	CONFINURE_CMD="--prefix=${THIRD_PATH} --enable-lib-only --host=${CROSS_PLATFORM}"
...
compile_zlib() {
	...
	if [ "$DUER_USE_HARD_FLOAT" = "yes" ]; then
...
compile_curl() {
	...
	CONFINURE_CMD="--enable-shared=yes --enable-static=no --prefix=${THIRD_PATH} --with-ssl=${THIRD_PATH} --host=${CROSS_PLATFORM} --with-zlib=${THIRD_PATH} --with-nghttp2=${THIRD_PATH}"
...
compile_ffmpeg() {
   ...
   CONFINURE_CMD="${CONFINURE_CMD} --disable-static --enable-shared --disable-programs --target-os=linux --incdir=${THIRD_PATH}/include/ --libdir=${THIRD_PATH}/lib --disable-doc"
   ...
   #if [ "${Platform}" = "Ingenic" ] || [ "${Platform}" = "Cchip" ]; then
        patch < ${WORK_PATH}/patchs/ffmpeg_ingenic.patch
        CONFINURE_CMD="${CONFINURE_CMD} --cpu=xburst"
   else
        CONFINURE_CMD="${CONFINURE_CMD} --cpu=generic --disable-mipsfpu"
   fi
