...
#include "Others/audioResample.h"
...
{                                                                           \
    if (!(condition))                                                       \
    {                                                                       \
        APP_ERROR(" [recorder error] %d: I am here %s:%d ", ret, __FUNCTION__, __LINE__);\
        return ret;                                                         \
    }                                                                       \
}while(0)
...
int AudioRecorder::openRecorder(struct recorder* rec) {
    ...
    do {
        ...
        if (err < 0) {
            ...
            APP_INFO("success open \"%s\" PCM device.", rec->alsa_rec_device);
        }
