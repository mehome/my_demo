...
void DeviceIoWrapper::callback(DeviceInput event, void *data, int len) {
    ...
    switch (event) {
        ...
        case DeviceInput::KEY_MIC_MUTE: {
			if(getMutMicStatus()){
				setMutMicStatus(0);
                SoundController::getInstance()->muteOn();
			}else{
				setMutMicStatus(1);
                SoundController::getInstance()->muteOff();
			}
//            handleSleepMode();
            break;
        }
        ...
        case DeviceInput::KEY_RT_VOLUME: {
            if (len == sizeof(int)) {
                int value = *((int*)data);
                handleSetRTvolume(value);
            }
            break;
        }
...
//            }
            if (!m_isTouchStartNetworkConfig) {
				...
				if(data != NULL){
					int fail_type = *((int*)data);
					inf("fail_type: %d", fail_type);
					switch(fail_type){
						case 1:
							SoundController::getInstance()->network_fail_invalid_password();
							sleep(5);
							break;
						case 2:
							SoundController::getInstance()->network_fail_connect_router_failed();
							sleep(5);
							break;
						default:
							break;
...
void DeviceIoWrapper::handleVolumeDown(int dec) {
	...
	unsigned int volume=currentVolume - dec;
//	cdb_set_int("$ra_vol", volume);
	uartd_fifo_write_fmt("setvol%03u",volume);
...
void DeviceIoWrapper::handleVolumeUp(int inc) {
    SoundController::getInstance()->volume();
    int currentVolume = getCurrentVolume();
	unsigned int volume=currentVolume + inc;
//	cdb_set_int("$ra_vol", volume);
	uartd_fifo_write_fmt("setvol%03u",volume);
...
void DeviceIoWrapper::led_first_poweron(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::FIRST_POWERON);
}
void DeviceIoWrapper::led_connect_start(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::CONNECT_START);
}
void DeviceIoWrapper::led_connecting(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::CONNECTING);
}
void DeviceIoWrapper::led_connect_succeed(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::CONNECT_SUCCEED);
}
void DeviceIoWrapper::led_wrong_password(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::WRONG_PASSWORD);
}
void DeviceIoWrapper::led_unsupported_route(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::UNSUPPORTED_ROUTE);
}
void DeviceIoWrapper::led_connection_routing_failed(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::CONNECTION_ROUTING_FAILED);
}
void DeviceIoWrapper::led_device_boot(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::DEVICE_BOOT);
}
void DeviceIoWrapper::led_device_reboot(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::DEVICE_REBOOT);
}
void DeviceIoWrapper::led_factory_set(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::FACTORY_SET);
}
void DeviceIoWrapper::led_turn_down_the_volume(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::TURN_DOWN_THE_VOLUME);
}
void DeviceIoWrapper::led_pardon_again(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::PARDON_AGAIN);
}
void DeviceIoWrapper::led_network_fault(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::NETWORK_FAULT);
}
void DeviceIoWrapper::led_binding_success(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::BINDING_SUCCESS);
}
void DeviceIoWrapper::led_network_exception(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::NETWORK_EXCEPTION);
}
void DeviceIoWrapper::led_slow_network(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::SLOW_NETWORK);
}
void DeviceIoWrapper::led_login_failure(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::LOGIN_FAILURE);
}
void DeviceIoWrapper::led_wait_moment(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::WAIT_MOMENT);
}
void DeviceIoWrapper::led_idel(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::IDEL);
}
void DeviceIoWrapper::led_wakeup(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::WAKEUP);
}
void DeviceIoWrapper::led_listening(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::LISTENING);
}
void DeviceIoWrapper::led_thinking(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::THINKING);
}
void DeviceIoWrapper::led_talking(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::TALKING);
}
void DeviceIoWrapper::led_play_listen(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::PLAY_LISTEN);
}
void DeviceIoWrapper::led_play_silence(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::PLAY_SILENCE);
}
void DeviceIoWrapper::led_tip_sound_start(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::TIP_SOUND_START);
}
void DeviceIoWrapper::led_tip_sound_end(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::TIP_SOUND_END);
}
void DeviceIoWrapper::led_enter_config_net_mode(void* args){
	uartd_fifo_write_fmt("duerSceneLedSet%03u",(unsigned int)duerScene_t::ENTER_CONFIG_NET_MODE);
}
#if 0
...
void DeviceIoWrapper::wakeupVolumeDown() {
    int currentVolume = getCurrentVolume();
	unsigned int volume=currentVolume - 10;
//	cdb_set_int("$ra_vol", volume);
	uartd_fifo_write_fmt("setvol%03u",volume);
...
std::string DeviceIoWrapper::getBtMac() {
    char bt_mac[18] = {0};
    DeviceIo::getInstance()->controlBt(BtControl::GET_BT_MAC, bt_mac,sizeof(bt_mac));
    return std::string(bt_mac);
}

std::string DeviceIoWrapper::getWifiMac() {
    char wifi_mac[18] = {0};
    DeviceIo::getInstance()->controlBt(BtControl::GET_WIFI_MAC, wifi_mac,sizeof(wifi_mac));
    return std::string(wifi_mac);
}

std::string DeviceIoWrapper::getWifiIp() {
    char wifi_ip[40] = {0};
    DeviceIo::getInstance()->controlBt(BtControl::GET_WIFI_IP, wifi_ip,sizeof(wifi_ip));
...
std::string DeviceIoWrapper::getWifiSsid() {
	...
	inf(__func__);
    DeviceIo::getInstance()->controlBt(BtControl::GET_WIFI_SSID, wifi_ssid,sizeof(wifi_ssid));
	inf(__func__);
...
std::string DeviceIoWrapper::getWifiBssid() {
    ...
    DeviceIo::getInstance()->controlBt(BtControl::GET_WIFI_BSSID, wifi_bssid,sizeof(wifi_bssid));
