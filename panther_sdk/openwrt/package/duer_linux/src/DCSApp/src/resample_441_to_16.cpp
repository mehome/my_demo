...
// 小端
void Resample::ConvertFreq(short *source, long srcIdx, short *target, long tgtIdx,long coefIdx)
{
    //long j;
 		// long double temp;
  	int j;
...
void Resample::resample_buf_441_to_16(char * in,unsigned long len,char * out, unsigned long *length)
{	
  ...
  //APP_INFO("len=%lu",len);  
   ...
   //APP_INFO("*out len=%lu",len*160/441/2);  

  inFileData = (short *)malloc((BUFFER_HEADER + BUFFER_SIZE)*sizeof(short));
  
  outFileData = (short *)malloc((BUFFER_SIZE)*sizeof(short));
   
  inFileData_tmp = (short *)malloc(2*BUFFER_SIZE*sizeof(short));
   ...
   do
   {
      if(source_type ==2) {
	  		...
	  		if(len > BUFFER_SIZE*2*2) {
	  				APP_INFO("len > BUFFER_SIZE*2*2",len);
	  				count = BUFFER_SIZE*2;
	  				len -= count*2;
	  		} else {
	  				count = len/2;
  		...
  		out += ((count)*160/441*2);
  		//APP_INFO("length1=%lu",((count)*160/441*2));
  		*length += ((count)*160/441*2);
  		//APP_INFO("length=%lu",*length);
