...
bool AlsaController::setParams(unsigned int rate, unsigned int channels) {
    ...
    ret = snd_pcm_hw_params(m_pcmHandle, params);		//所有的参数设置后，经过这里之后才会起生效
    if (ret < 0) {
        return false;
    }
	...
	int bits_per_sample = snd_pcm_format_physical_width(SND_PCM_FORMAT_S16_LE);		//每个采样需要16bits即2BYTEs
	m_chunkBytes = chunk_size * bits_per_sample / 8;
	m_bits_per_frame = bits_per_sample * channels;
	dbg("chunBytes = %d, chunk_size = %d, bits_per_frame=%d", m_chunkBytes, chunk_size, m_bits_per_frame );
	return true;
}
...
//	dbg("myStopWriteiFlag=%d",getStopWriteiFlag());
	while ( c > 0 && !getStopWriteiFlag()) {
