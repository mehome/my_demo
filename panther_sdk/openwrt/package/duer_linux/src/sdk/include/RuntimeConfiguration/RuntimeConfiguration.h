 ...
 * \if english
 *  @brief the RuntimeConfiguration node can be obtained by this interface
 * \else
 *  @brief 应用层可以通过这个接口获取配置运行节点信息
 * \endif
 *
 */
...
#include "RuntimeConfigurationNode.h"
...
private:
     ...
     * \endif
     */
    static RuntimeConfiguration m_runtimeConfiguration;
...
protected:
    ...
    /**
     * \if english
     *  @brief the intenal mutex map
     * \else
     *  @brief 内部mutex map表
     * \endif
     */
    static std::map<std::string, std::mutex*> m_mutexMap;
};
