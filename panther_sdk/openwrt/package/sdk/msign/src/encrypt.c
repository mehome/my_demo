...
int _encrypt(const char *infile, const char *outfile, int page_size, int start_offset);
...
int gen_decrypt_erased_page_header(const char *outfile, int page_size)
{
   ...
   fprintf(filew, "};\n\n");
   fprintf(filew, "\nstatic inline int is_erased_page(unsigned char *data)\n");
   fprintf(filew, "{\n");
   fprintf(filew, "  int i;\n");
   fprintf(filew, "  if(memcmp(__erased_page_data_first, &data[0], 16))\n");
   fprintf(filew, "     return 0;\n");
   fprintf(filew, "  for(i=16;i<ERASED_PAGE_SIZE;i+=16)\n");
   fprintf(filew, "  {\n");
