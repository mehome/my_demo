...
int wfaStaGetIpConfig(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
     ...
     */
    ret = access("./scripts/getipconfig.sh", F_OK);
    if(ret == -1)
    {
     ...
     */
    sprintf(gCmdStr, "./scripts/getipconfig.sh /tmp/ipconfig.txt %s\n", ifname);
...
int wfaStaGetMacAddress(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    tmpfd = fopen("/tmp/ipconfig.txt", "rw+");
    if(tmpfd == NULL)
    {
...
int wfaSetEncryption(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    sprintf(gCmdStr, "wpa_cli -i %s remove_network 0", setEncryp->intf);
    sret = call_system(gCmdStr);
    sprintf(gCmdStr, "wpa_cli -i %s add_network 0", setEncryp->intf);
    sret = call_system(gCmdStr);
...
int wfaStaSetPSK(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    sprintf(gCmdStr, "wpa_cli -i %s remove_network 0", setPSK->intf);
    sret = call_system(gCmdStr);
    sprintf(gCmdStr, "wpa_cli -i %s add_network 0", setPSK->intf);
    sret = call_system(gCmdStr);
    ...
    else if(strcasecmp(setPSK->keyMgmtType, "wpa2") == 0)
    {
        ...
        //sprintf(gCmdStr, "wpa_cli -i %s set_network 0 key_mgmt WPA2", setPSK->intf);
    ...
    {
        sprintf(gCmdStr, "wpa_cli -i %s set_network 0 key_mgmt WPA2-FT", setPSK->intf);
    }
    ...
    if (setPSK->encpType == ENCRYPT_AESCCMP) {
        ...
        sret = call_system(gCmdStr);
        sprintf(gCmdStr, "wpa_cli -i%s set_network 0 pairwise CCMP", setPSK->intf);
        sret = call_system(gCmdStr);
    } else if(setPSK->encpType == ENCRYPT_TKIP) {
        sprintf(gCmdStr, "wpa_cli -i%s set_network 0 proto WPA", setPSK->intf);
        sret = call_system(gCmdStr);
        sprintf(gCmdStr, "wpa_cli -i%s set_network 0 pairwise TKIP", setPSK->intf);
        sret = call_system(gCmdStr);
        ...
        printf("sneeker: not supported encryption type\n");
    }
...
int wfaStaPresetParams(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
   ...
   if (presetParams->supplicant == eWpaSupplicant)
   {
	...
	sprintf(cmdStr, "ps ax | grep wpa_supplicant >> /tmp/processid.txt\n");
	//sprintf(cmdStr, "/usr/local/sbin/findprocess.sh %s /tmp/processid.txt\n", "wpa_supplicant");
	st = call_system(cmdStr);
...
int wfaStaSendADDBA(int len, BYTE *caCmdBuf, int *respLen, BYTE *respBuf)
{
    ...
    printf("sneeker: %s() is start (status : %d)\n", staSendADDBAResp->status);
    sprintf(gCmdStr, "echo \"addba 0 0 0\" > /proc/lynx");
    sret = call_system(gCmdStr);
...
//    char *addr = staPing->dipaddr;
    ...
    {
        if ( tos>0) {
//          sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;./scripts/wfaping6.sh %s %s -i %f -c %i -Q %d -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
//                  streamid,streamid,bflag, staPing->dipaddr, *interval, totalpkts, tos,  staPing->frameSize,streamid);
            printf("\n#### TODO: not support tos,interval of busybox in openwrt ####\n");
            sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;./scripts/wfaping6.sh %s %s -c %i -Q %d -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
                    streamid,streamid,bflag, staPing->dipaddr, totalpkts, tos,  staPing->frameSize,streamid);
        } else {
            printf("\n#### TODO: not support interval of busybox in openwrt ####\n");
            sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;./scripts/wfaping6.sh %s %s -c %i -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
                    streamid,streamid,bflag, staPing->dipaddr, totalpkts, staPing->frameSize,streamid);
        }
        sret = call_system(cmdStr);
        printf("\nCS : The command string is %s",cmdStr);
    }
    else
    {
        if (tos > 0){
//          sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;./scripts/wfaping.sh %s %s -i %f -c %i  -Q %d -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
//                  streamid,streamid,bflag, staPing->dipaddr, *interval, totalpkts, tos, staPing->frameSize,streamid);
            printf("\n#### TODO: not support tos,interval of busybox in openwrt ####\n");
            sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;./scripts/wfaping.sh %s %s -c %i  -Q %d -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
                    streamid,streamid,bflag, staPing->dipaddr, totalpkts, tos, staPing->frameSize,streamid);
        } else {
//          sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;./scripts/wfaping.sh %s %s -i %f -c %i -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
//                  streamid,streamid,bflag, staPing->dipaddr, *interval, totalpkts, staPing->frameSize,streamid);
            printf("\n#### TODO: not support interval of busybox in openwrt ####\n");
            sprintf(cmdStr, "echo streamid=%i > /tmp/spout_%d.txt;./scripts/wfaping.sh %s %s -c %i -s %i -q >> /tmp/spout_%d.txt 2>/dev/null",
                    streamid,streamid,bflag, staPing->dipaddr, totalpkts, staPing->frameSize,streamid);
        }
        sret = call_system(cmdStr);
        printf("\nCS : The command string is %s",cmdStr);
...
int wfaStopPing(dutCmdResponse_t *stpResp, int streamid)
{
    ...
    sret = call_system("./scripts/stoping.sh /tmp/pid.txt ; sleep 2");
