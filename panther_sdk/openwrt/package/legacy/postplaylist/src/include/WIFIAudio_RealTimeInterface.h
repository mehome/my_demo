...
{
	char *pRet;				// 成功标志
	int MainMode;				// 主模式: 0-普通音响或者主音箱 1-子音响
	int NodeType;				// 声道：0-表示立体声，1-表示左声道，2-表示右声道
	int Mode;				// 播放模式：0-表示无模式，1-表示第三方软件推送，2-表示Airplay，3-表示U盘本地播放，4-表示wiimu的列表推送模式
	int SW;					// 播放列表中的歌曲切换模式：0-顺序播放，1-循环单曲，2-随机播放，3-列表循环
	char *pStatus;				// 当前状态
	int CurPos;				// 当前的位置
	int TotLen;				// 总长度
	char *pTitle;				// 标题
