...
/*
	...
	*/
	...
	printf("result = %d, EOF = %d, status = %ld, OFF = %lld, SIZE = %lld\n", result, IsEOF(), status, (long long)offset , (long long)size );
	if( CURLE_PARTIAL_FILE == result && !IsEOF() ) {
		reconnect = 1;
		cdb_set_int("$mpd_playing", 1);
		ReConnect(offset);
	} else {
	...
	if (result != CURLE_OK) {
		postponed_error.Format(curl_domain, result,
		       "mpd curl failed: %s", error_buffer);
	//	cdb_set_int("$mpd_playing", 0);
...
CurlMulti::TimerFunction(gcc_unused CURLM *_multi, long timeout_ms, void *userp)
{
	...
	easy = (CURL *)multi.get();
	long status = 0;
	CurlInputStream *p = input_curl_find_request(easy);
	if(p && cmd_trig) {
		p->CheckPending();
		cmd_trig = false;
	}
...
input_curl_writefunction(void *ptr, size_t size, size_t nmemb, void *stream)
{
	...
	if ( (mime != nullptr) && 
		...
		foreach(line, p, next)
		{
			...
			else if( !strncmp(line, "[playlist]", 10) || !strncmp(line, "NumberOfEntries", 15) ) {
...
CurlInputStream::InitEasy(Error &error)
{
	...
	curl_easy_setopt(easy, CURLOPT_CONNECTTIMEOUT, 40l);

#ifdef MONTAGE_MPD_PATCH
	 /* enable TCP keep-alive for this transfer */
	curl_easy_setopt(easy, CURLOPT_TCP_KEEPALIVE, 1l);
	/* keep-alive idle time to 120 seconds */
	curl_easy_setopt(easy, CURLOPT_TCP_KEEPIDLE, 20l);
	/* interval time between keep-alive probes: 60 seconds */
	curl_easy_setopt(easy, CURLOPT_TCP_KEEPINTVL, 3l);
#endif
	...
	CURLcode code;
	if(reconnect) {
		code = curl_easy_setopt(easy, CURLOPT_URL, real_uri.empty() ? GetURI() : real_uri.c_str() );
	} else {
		code = curl_easy_setopt(easy, CURLOPT_URL, GetURI());
	}
	if (code != CURLE_OK) {
		error.Format(curl_domain, code,
		      ...
		      Error &error)
{
	...
	/*
	suffix = uri_get_suffix(url);
	suffix = strstr(url);
	if( StringEqualsCaseASCII(suffix, "m3u") || StringEqualsCaseASCII(suffix, "m3u8") || StringEqualsCaseASCII(suffix, "pls") ) {
		c->redirect = true;
	}
*/	
	return c->icy;
...
#ifdef MONTAGE_MPD_CMD_TRIGGER
inline void
CurlInputStream::CheckPending(void)
{
        const ScopeLock protect(mutex);
