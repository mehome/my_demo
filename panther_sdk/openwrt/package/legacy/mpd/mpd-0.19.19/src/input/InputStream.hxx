...
private:
	...
	/**
	 * Montage Patch, the redirect uri, or empty if not in use.
	 */
...
public:
	...
	virtual bool HasRedirURI() const {
		return !redir_uri.empty();
	}

	gcc_nonnull_all
	void SetRedirURI(const char *_redir_uri) {
		redir_uri = _redir_uri;
		int n = redir_uri.find_last_of('\n');
		if( n >= 0 ) 
			redir_uri.erase(n);
	}

	virtual const char *GetRedirURI() const {
		return redir_uri.empty() ? nullptr : redir_uri.c_str();
	}
