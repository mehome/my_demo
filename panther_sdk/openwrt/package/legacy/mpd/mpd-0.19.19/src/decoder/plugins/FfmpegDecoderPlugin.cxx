...
static const char *mpd_montage = nullptr;
static const char *mpd_probesize = nullptr;
static const char *mpd_analyzeduration = nullptr;
#endif
...
FfmpegOpenInput(AVIOContext *pb,
		const char *filename,
		AVInputFormat *fmt, InputStream &input,
		Error &error)
{
	...
	AVDictionary *opts = 0;
	int mpd_probesize = 0;
	int mpd_analyzeduration = 0;
	const char *uri = input.GetURI();
		
	mpd_probesize = cdb_get_int("$mpd_probesize", 0);
	if( mpd_probesize > 0 ) {
			av_dict_set_int(&opts, "probesize", mpd_probesize, 0);
	}
	mpd_analyzeduration = cdb_get_int("$mpd_duration", 0 );
	if( mpd_analyzeduration > 0 ) {
			av_dict_set_int(&opts, "analyzeduration", mpd_analyzeduration, 0);	
	}
	
	if( strstr(uri, "://") == nullptr ) {
		context->probesize = 16 * 1024;
  		context->max_analyze_duration = 1 * AV_TIME_BASE;
		av_dict_set(&opts, "analyzeduration", "1000", 0);
		av_dict_set(&opts, "timeout", "1000", 0);
		context->flags |= AVFMT_FLAG_NONBLOCK;
		context->flags |= AVFMT_FLAG_NOBUFFER;
	}	
	
	int err = avformat_open_input(&context, filename, fmt, &opts);
...
{
#ifdef MONTAGE_MPD_PATCH
	if (param.GetBlockValue("montage", 0) != 0)
		mpd_montage = param.GetBlockValue("montage");
	if (param.GetBlockValue("probesize", 0) != 0)
		mpd_probesize = param.GetBlockValue("probesize");
	if (param.GetBlockValue("analyzeduration", 0) != 0)
		mpd_analyzeduration = param.GetBlockValue("analyzeduration");

	FormatWarning(ffmpeg_domain, "montage=[%s], probesize=[%s], analyzeduration=[%s]", 
		mpd_montage, mpd_probesize, mpd_analyzeduration);
		   ...
		   FfmpegBuffer &buffer)
{
	...
	while (packet.size > 0 && cmd == DecoderCommand::NONE) {
		...
		/* added by tashin, for exception catch */
		try {
				len = avcodec_decode_audio4(&codec_context,
						&frame, &got_frame,
						&packet);
		}
...
#else
	/* it's neccessary for seeking of aac streaming , test stable */
	avcodec_copy_context(codec_context, av_stream.codec);
#endif
#endif
	...
	const SignedSongTime total_time =
		...
		: ( (format_context.duration != (int64_t)AV_NOPTS_VALUE) ? 
			 SignedSongTime::FromScale<uint64_t>(format_context.duration,
						      AV_TIME_BASE) : SignedSongTime::Negative() );
#endif
...
ffmpeg_decode(Decoder &decoder, InputStream &input)
{
	...
	Error error;
	AVFormatContext *format_context =
		FfmpegOpenInput(stream.io, input.GetURI(), input_format, input, error);
		   ...
		   const struct tag_handler *handler, void *handler_ctx)
{
	...
	AVFormatContext *f =
		FfmpegOpenInput(stream.io, is.GetURI(), input_format, is,
				IgnoreError());
	if (f == nullptr)
		return false;
...
#ifdef MONTAGE_MPD_PATCH
	"m3u8","m3u",
	"aac", "amr", "ape", "flac", "m4a", "mp4", "wav", "wma", "ogg", "mp3",
	nullptr
#else
