...
decoder_run_stream(Decoder &decoder, const char *uri)
{
	...
	if (input_stream == nullptr) {
		dc.Lock();
		dc.reason = DecoderReason::DECODE_REASON_FILE;
		return false;
	}
		 ...
		 const DetachedSong &song, const char *uri, Path path_fs)
{
	...
	if (decoder.error.IsDefined()) {
		...
		if(dc.reason == DecoderReason::DECODE_REASON_FILE)
			dc.error.Format(decoder_domain,
					 "Cannot Access %s", error_uri);
		else
			dc.error.Format(decoder_domain,
...
decoder_thread_start(DecoderControl &dc)
{
	...
	Error error;
	if (!dc.thread.Start(decoder_task, &dc, 0, 1536*1024, error))
		FatalError(error);
}
