...
public:
	...
	#ifdef MONTAGE_MPD_PATCH
#ifdef ENABLE_CODEC_ES8316
		volume_max = cdb_get_int("$hw_vol_max", 192);
		volume_min = cdb_get_int("$hw_vol_min", 100);
#else
		volume_max = cdb_get_int("$hw_vol_max", 255);
		volume_min = cdb_get_int("$hw_vol_min", 155);
#endif
		volume = cdb_get_int("$ra_vol", 60);
	#endif
	}
...
{
#ifndef MONTAGE_MPD_PATCH
	assert(new_volume <= 100);	
	volume = new_volume;
	volume_filter_set(filter, PercentVolumeToSoftwareVolume(new_volume));
#else
	assert(new_volume <= 200);
	if (new_volume <= 100)	{
		set_system_volume(new_volume);
	}else if (new_volume > 100 && new_volume <= 200)	{
		unsigned vol;
		vol  = new_volume - 100;
		volume_filter_set(filter, PercentVolumeToSoftwareVolume(vol));	
	}
#endif
	return true;
}
