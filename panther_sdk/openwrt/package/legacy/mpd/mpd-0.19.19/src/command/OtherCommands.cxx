...
#ifdef MONTAGE_MPD_PATCH
//	if (Myctv) {
//		unsigned volume = (unsigned)cdb_get_int("$ra_vol", 0);
//		if (level != volume) {
//			cdb_set_int("$ra_vol", level);
//			Myctv->ScheduleSeconds(5);
//		}
//	}
#endif
...
handle_volume(Client &client, gcc_unused unsigned argc, char *argv[])
{
	...
	if (!check_int(client, &relative, argv[1]))
		return CommandResult::ERROR;
	command_error(client, ACK_ERROR_ARG, "relative=%d",relative);
	if (relative < -100 || relative > 200) {
		command_error(client, ACK_ERROR_ARG, "Invalid volume value");
		return CommandResult::ERROR;
	}
