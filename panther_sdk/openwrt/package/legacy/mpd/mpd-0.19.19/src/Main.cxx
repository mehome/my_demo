...
static constexpr unsigned DEFAULT_BUFFER_SIZE = 2560;
...
extern void mon_exit();
#endif


const Domain cdb_domain("cdb");

void CdbTimer::OnTimeout()
{
	assert(GetEventLoop().IsInside());

    switch(m_type) {
        case CDB_TIMER_VOLUME:
        {
			struct stat buf;

        	FormatInfo(cdb_domain, "Time to update Volume");

        	/* save volume value, maybe it will be set next boot */
        	if (stat("/lib/wdk/save", &buf) == 0) {
        		system("/lib/wdk/save");
        		FormatInfo(cdb_domain, "cdb save!");
        	}
            break;
        }
        default:
...
#ifdef MONTAGE_MPD_PATCH
{
	Myctv = new CdbTimer(*instance->event_loop, CDB_TIMER_VOLUME);
	if (!Myctv) {
		FormatErrno(cdb_domain, "alloc Myctv is failed!");
		return EXIT_FAILURE;
	}
}
#endif
...
#ifdef MONTAGE_MPD_PATCH
	if (create_db) {
		if (config_get_bool(CONF_UPDATE_DB_START, false)) {
			...
			FormatWarning(config_domain, "bypass create database when start");
		}
	}
#else
	if (create_db) {
