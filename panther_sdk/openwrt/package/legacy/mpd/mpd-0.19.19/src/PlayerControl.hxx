...
struct PlayerControl {
...
#ifdef TURING_MPD_CHANGE
	     std::atomic_uint turing_state;
		//TuringState turing_state ; 
#endif
	...
	bool WaitTimeout(int timeout_ms) {
		...
		return cond.timed_wait(mutex, timeout_ms);
	}
#endif
	...
	void ClientWait() {
		...
		client_cond.timed_wait(mutex, 6000);
	}
...
#ifdef TURING_MPD_CHANGE
	unsigned GetTuringState()   {
		return turing_state.exchange(0);
	}
	void SetTuringState(TuringState _state)  {
		assert((unsigned)_state < TuringState::MAX);

		const unsigned mask = 1u << unsigned(_state);
		turing_state.fetch_or(mask);
	}
#endif
	/**
