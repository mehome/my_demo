...
static int OnWriteMsg(char *path,  char* cmd)
{
	...
	if(iConnect_fd<0)
	{
		...
		FormatDefault(player_domain, "cannot create communication socket");
		return 1;
	}
	...
	if(0 != connect(iConnect_fd, (struct sockaddr*)&srv_addr, sizeof(srv_addr)))
	{
		...
		FormatDefault(player_domain, "connect server failed..");
		return 1;
	}
	...
	if (iRet != strlen(cmd))
	{
		FormatDefault(player_domain, "write server failed..");
		iRet = -1;
	}
	else
	{
...
Player::WaitForDecoder()
{
	...
	delete song;
	song = pc.next_song;
#ifdef TURING_CTRL_PLAYER 
	cdb_set("$turing_song_name",(song->GetSongName() == NULL) ? (void *)"0" :(void *)song->GetSongName());
	//pc.Lock();
...
Player::OpenOutput()
{
	...
	if (pc.outputs.Open(play_audio_format, buffer, error)) {
		...
		//system("uartdfifo.sh play");
		cdb_set_int("$current_play_state", 1);
		pc.Unlock();
		...
		pc.state = PlayerState::PAUSE;
		//system("uartdfifo.sh pause"); 
		cdb_set_int("$current_play_state", 2);
		pc.Unlock();
...
#ifndef MONTAGE_MPD_CMD_TRIGGER
...
AGAIN:
				dc.Signal();
				if(false == dc.WaitForDecoderTimeout(100))
				{
					pc.outputs.Check();
					max_retry--;
					if(max_retry>=0)
...
Player::Run()
{
	...
	pc.Lock();
#ifdef MONTAGE_MPD_CMD_TRIGGER
	pc.state = PlayerState::BUFFERING;
#else
	pc.state = PlayerState::PLAY;
#endif
	...
	while (true) {
		...
		if (decoder_starting) {
#ifdef MONTAGE_MPD_CMD_TRIGGER
			if(pc.state == PlayerState::BUFFERING)
				pc.state = PlayerState::PLAY;
#endif
...
AGAIN:
...
#ifdef MONTAGE_MPD_CMD_TRIGGER
	if((pc.command == PlayerCommand::CLOSE_AUDIO) || 
			(pc.command == PlayerCommand::STOP)) {
        	cmd_trig = true;
	}
#endif
	...
	if(mode == 1)
	{
		if ((ret = OnWriteMsg("/tmp/UNIX.turing", "PlayDone")) != 0)
		{
			printf("OnWriteMsg failed\n");
		}
	} else if(mode == 3) {
		const char *url = song->GetURI();
		if(strstr(url , "_mc.mp3") == NULL) {
			OnWriteMsg("/tmp/UNIX.turing", "HuabeiPlayStop");
	...
	{
		if ((ret = OnWriteMsg("/tmp/UNIX.turing", "MpcStopEvent")) != 0)
		{
			printf("OnWriteMsg failed\n");
		}
	}
	//cdb_set("$turing_song_name", (void *)"0");
#endif
	StopDecoder();
...
StartPlayerThread(PlayerControl &pc)
{
	...
	Error error;
	if (!pc.thread.Start(player_task, &pc, 0, 2048*1024, error))
		FatalError(error);
}
