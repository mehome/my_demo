...
UpdateService::Task()
{
	...
	{
	    strcpy(buf,next.path_utf8.c_str());
	...
	if (!next.path_utf8.empty())
	{
    	...
    	if(buf[0] == 's')
    	{
            cdb_set_int("$usb_scan_over", 1);
    	}
    	else if(buf[0] == 'm')
    	{
            cdb_set_int("$tf_scan_over", 1);
    	}
    	
		FormatDebug(update_domain, "finished: %s",next.path_utf8.c_str());
    }
	else
...
UpdateService::StartThread(UpdateQueueItem &&i)
{
	...
	Error error;
	if (!update_thread.Start(Task, this, 0, 1024*1024, error))
		FatalError(error);
