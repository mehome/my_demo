...
#ifdef MONTAGE_MPD_PATCH
	songs_nr += directory.songs.size();
	NotifySongsNumber(songs_nr);
#endif
...
#ifdef MONTAGE_MPD_PATCH
	NotifySongsNumber(0); // finished to update
#endif
...
#ifdef MONTAGE_MPD_PATCH
void
UpdateWalk::NotifySongsNumber(unsigned num)
{
	struct stat buf;
	char cmd[32];

	if (stat("/etc/mpd_updated.sh", &buf) == 0) {
		sprintf(cmd, "/etc/mpd_updated.sh %d", num);
		system(cmd);
	}
}
#endif
