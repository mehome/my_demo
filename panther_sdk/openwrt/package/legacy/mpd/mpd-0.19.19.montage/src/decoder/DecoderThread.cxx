...
Open:
	if (Redirect_uri.empty())
		is = InputStream::Open(uri, dc.mutex, dc.cond, error);
	else {
		printf("[FLOW_DEUBG][%s:%s]----------redirect to open input stream %s----------\n", __FILE__, __func__, Redirect_uri.c_str());
		is = InputStream::Open(Redirect_uri.c_str(), dc.mutex, dc.cond, error);
		Redirect_uri.clear();
	}

	if (is == nullptr) {
		printf("[FLOW_DEUBG][%s:%s]----------InputStream Open failed----------\n", __FILE__,  __func__);
		if (error.IsDefined())
			LogError(error);
	...
	if (!is->Check(error)) {
		...
		printf("[FLOW_DEUBG][%s:%s]----------InputStream Check failed----------\n", __FILE__, __func__);
		LogError(error);
		return nullptr;
	}
...
static bool
decoder_run_stream_plugin_mime(Decoder &decoder, InputStream &is,
			  const char *suffix,
			  const DecoderPlugin &plugin,
			  bool &tried_r)
{
	printf("[FLOW_DEUBG][%s:%s]----------%s----------\n", __FILE__, __func__, plugin.name);
...
static bool
decoder_run_stream_plugin_suffix(Decoder &decoder, InputStream &is,
			  const char *suffix,
			  const DecoderPlugin &plugin,
			  bool &tried_r)
{
	printf("[FLOW_DEUBG][%s:%s]----------%s----------\n", __FILE__, __func__, plugin.name);
	if (!decoder_check_plugin_suffix(plugin, suffix))
		return false;
			  ...
			  const char *uri, bool &tried_r)
{
	...
	printf("[FLOW_DEUBG][%s:%s]----------get suffix %s----------\n", __FILE__, __func__, suffix);

	using namespace std::placeholders;
	const auto f_suffix = std::bind(decoder_run_stream_plugin_suffix,
				 std::ref(decoder), std::ref(is), suffix,
				 _1, std::ref(tried_r));
	const auto f_mime = std::bind(decoder_run_stream_plugin_mime,
				 std::ref(decoder), std::ref(is), suffix,
				 _1, std::ref(tried_r));

	return (decoder_plugins_try(f_suffix) || decoder_plugins_try(f_mime));
}
