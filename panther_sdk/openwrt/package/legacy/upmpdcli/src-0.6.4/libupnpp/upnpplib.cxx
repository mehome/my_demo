	...
	: m_ok(false)
{
	...
	string temp_str;
	temp_str.assign(ip_address[0] ? ip_address : "0");
...
int LibUPnP::setupWebServer(const string& description)
{
	...
	res = UpnpSendAdvertisement(m_dvh, 1800);

	if (res != UPNP_E_SUCCESS)
		LOGERR(errAsString("Error sending advertisements", m_init_error) << endl);
...
char *boot_cdb_get_st(char *name, char *val)
{
	 FILE *fp = NULL;
	 char *p = NULL, *eq = NULL, buf[256];
...
string LibUPnP::makeDevUUID(const std::string& name, const std::string& hw)
{
	...
	*/	
	...
	//cdb_get_st("mac2", mac);
	cdb_get_str("$boot_mac2",mac,sizeof(mac),"");
//  del_str(mac);
sscanf(mac, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", 
        &val[0], &val[1], &val[2], &val[3], &val[4], &val[5]);
	/*sprintf(uuid, "%s-%s-%s-%s-%s",
            "xzxaudio", "wifi", "1511","0023", val);
            */
  sprintf(uuid, "xzxaudio-wifi-1511-0023-%02x%02x%02x%02x%02x%02x",val[0], val[1], val[2], val[3],val[4], val[5]);
	    std::cout << "uuid 0000000000000 ==" << uuid << std::endl;
	//char tempbuff[128];
//	memset(tempbuff, 0, sizeof(tempbuff)/sizeof(char));	
	cdb_set("$wifiaudio_uuid", uuid);
	//sprintf(tempbuff,"creatPlayList modifyConf \"%s\" \"%s\"","wifiaudio_uuid",uuid);
     //   std::cout << "tmp uuid ==" << tempbuff << std::endl;
	//system(tempbuff);
	//memset(tempbuff, 0, sizeof(tempbuff)/sizeof(char));	
//	sprintf(tempbuff,"creatPlayList modifyConf \"%s\" \"%s\"","xzxwifiaudio.config.mac_addr",hw.c_str());
	//system(tempbuff);
	return uuid;
}
