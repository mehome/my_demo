...
bool AudioDecoder::open(const std::string &url) {
    m_bytesPersample = 0;
    m_currentTimes = 0;
	int err=avformat_open_input(&m_formatCtx,
                            url.c_str(),
                            NULL,
                            NULL) ;
	if(err){
		myPlayErr("%s open falure,err=%d",url.c_str(),err);
		return false;
	}
    ...
    av_dump_format(m_formatCtx, 0, url.c_str(), false);		//调试函数
...
StreamReadResult AudioDecoder::readFrame(uint8_t **frameBuffer, unsigned int bufferLen, unsigned int *len) {
