		...
		else
		{
			...
			else
			{
				...
				for(i=0;i<25;i++)
				{
					...
					vcp_process_tx(reg,(pcm_t *)mic_in,(pcm_t *)ref_in,(pcm_t *)asp_out);
...
#if 1
extern PROFILE_TYPE(t) profile;
vcp_profile_t *p = &profile;
...
int vcp8_init(void)
{
    ...
    if (err.err)
    {
        if (err.err == ERR_INVALID_CRC)
            fprintf(stderr, "Profile error: Invalid CRC!");
        else
            fprintf(stderr, "Profile error: %s %d %d\n", __text_error[err.err], err.pid, err.memb);
        return -1;
    }
