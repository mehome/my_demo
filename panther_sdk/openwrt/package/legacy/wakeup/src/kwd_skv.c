...
void * skv_start_routine(void *args){
	skv_main(args);
}
...
|| defined(CONFIG_ASP_SQE) || defined(CONFIG_ASP_YSZ) || defined (CONFIG_ASP_VCP)
	...
	//	DEBUG_PRINTF("ASP_args samples:%d", samples);
#elif defined(CONEXANT_20921)
	...
	//	printf("22222222222222222222222\n");
		...
		//   printf("6666666666666\n");
		   ret = skv_awaken_run(sqe_buf, samples);		//检查是否唤醒,ret>0为唤醒了
		   //show_rt_time("", printf);
#elif defined(CONEXANT_20921)
            //hotword = SnowboyRunDetection((const int16_t *) asp_audiobuf, samples ,is_end);
			ret = skv_awaken_run(sqe_buf, samples);
#endif
	//	printf("ret 777777777777:%d\n",ret);
        if (ret > 0) 
        {
			//SnowboyReset();
			i++;
