...
//#define HBG_NETFILE      "ly_x_ni_hao_xiao_zhi_zh_sfs14_b24551b2_delivery03_am.raw"
#define HBG_NETFILE      "lp_x_xiaobiuxiaobiu_zhCN_sfs14_a4d81008_delivery02s_am.raw"
//#define HBG_SEARCHFILE   "thfft_alexa_enus_v3_250kb_search_12.raw" // pre-built search
...
//#define HBG_SEARCHFILE   "ly_x_ni_hao_xiao_zhi_zh_sfs14_b24551b2_delivery03_search_5.raw" // pre-built search
#define HBG_SEARCHFILE   "lp_x_xiaobiuxiaobiu_zhCN_sfs14_a4d81008_delivery02s_search_6.raw" // pre-built search
...
void * sensory_start_routine(void *args){
	sensory_main(args);
}
 static const char *rres;
 static thf_t *ses=NULL;
 static recog_t *r=NULL;
 static searchs_t *s=NULL;
 static pronuns_t *pp=NULL;
 static char str[MAXSTR];
 static const char *ewhere, *ewhat=NULL;
 static float score, delay;
 static unsigned short status, done=0;
 static audiol_t *p;
 static unsigned short i=0;
static void *cons=NULL;
#define inst NULL
int sensory_init()
{
...
#endif
  FILE* wakeloopStream=fopen("/tmp/wakeloop","w+");
  for (i=1; 1; i++) {
	rewind(wakeloopStream);
	fprintf(wakeloopStream,"%d\n",i);
...
#else
	WAKEUP_RAW("\n========wakeloop %d ===========\n",i);
	WAKEUP_INFO(">>> Say 'xiao biu,xiao biu' <<<");
#endif
    ...
    {      
		...
		if(!wakeupStartArgsPtr->isSensoryRecgnizeClosed)
		{
			thfRecogPipe(ses, r, samples, (short *) sqe_buf, RECOG_ONLY, &status);	
		}	
		if(wakeupStartArgsPtr->isMutiVadListening){
			if(wakeupStartArgsPtr->pRecordDataInputCallback){
				wakeupStartArgsPtr->timestamp = ts;
