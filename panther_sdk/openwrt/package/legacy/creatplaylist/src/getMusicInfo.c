...
unsigned short  GB2312_to_Unicode[][2] =
{
  0xA1A1, 0x3000,  /* '　' -> 12288 */
  0xA1A2, 0x3001,  /* '、' -> 12289 */
  0xA1A3, 0x3002,  /* '。' -> 12290 */
...
//??????????
int copy_binary_to_array(unsigned char *dest,unsigned char *src,unsigned int n,char *suffix)
{
	unsigned int i;
...
//只适合大段存储方式
int get_item_size_big(unsigned char buf[])
{
	return (buf[0]<<24) | (buf[1]<<16) | (buf[2]<<8) | buf[3];
}

//只适合小端方式
int get_item_size_little(unsigned char buf[])
{
...
//如果有ID3标志，返回1，出错返回-1，没有返回0
int check_if_have_id3(FILE *fp)
{
	int nread = 0;
	...
	{
		size = (buff[6]<<21) | (buff[7]<<14) | (buff[8]<<7) | buff[9];
		fseek(fp,size+10,SEEK_SET);		//跳过非法的ID3TAG信息
	}
	else
	{
		fseek(fp,0,SEEK_SET);
...
int UnicodeToUTF8(unsigned char *putf8, unsigned char high, unsigned char low)
{
	...
	int num = 0;
	unsigned short unicode = (high<<8) | low;
	
	if(NULL == putf8)
	{
		printf("Error of Parameter : NULL == putf8 in WIFIAudio_GetMP3Information_UnicodeToUTF8\r\n");
		num = 0;
	}
	else
	{
		...
		{
			//UFT-8单字节能表示的范围
			putf8[0] = unicode & 0x7f;
			num = 1;
		}
		else if(unicode <= 0x07ff)
		{
			//UFT-8双字节能表示的范围
			putf8[1] = 0x80 | (unicode & 0x3f);//10xxxxxx
			putf8[0] = 0xc0 | ((unicode>>6) & 0x1f);//110xxxxx
			num = 2;
		}
		else
		{
			//UFT-8三字节能表示的范围
			putf8[2] = 0x80 | (unicode & 0x3f);//10xxxxxx
			putf8[1] = 0x80 | ((unicode>>6) & 0x3f);//10xxxxxx
			putf8[0] = 0xe0 | ((unicode>>12) & 0x0f);//1110xxxx
			num = 3;
		}
	}
...
int UnicodeStringToUTF8String(unsigned char *putf8string, unsigned char *punicodestring, int num)
{
	...
	if((NULL == putf8string) || (NULL == punicodestring))
	{
		PRINTF("Error of Parameter : NULL == putf8string or NULL == punicodestring in WIFIAudio_GetMP3Information_UnicodeStringToUTF8String\r\n");
	}
	else
	{
		if((0xff == punicodestring[0])&&((0xfe == punicodestring[1])))					//小端，低位字节存在低地址，先读到的是高低位
		{
			for(i = 2; i < num; i = i + 2)
			{
				memset(utf8, 0, sizeof(utf8)/sizeof(unsigned char));
				ret = UnicodeToUTF8(utf8, punicodestring[i + 1], punicodestring[i]);
				
				append_binary_to_array(putf8string, utf8, ret,writen_num);
				writen_num += ret;
			}
		}
		else if((0xfe == punicodestring[0])&&((0xff == punicodestring[1])))				//大端，高位字节存在低地址，先读到的是高位
		{
			for(i = 2; i < num; i = i + 2)
			{
				memset(utf8, 0, sizeof(utf8)/sizeof(unsigned char));
				ret = UnicodeToUTF8(utf8, punicodestring[i], punicodestring[i + 1]);
...
static unsigned short binary_search(unsigned short x)
{ 
	int left = 0;
	...
	while(left <= right)
	{
		...
		if (x == GB2312_to_Unicode[middle][0]) 
			return GB2312_to_Unicode[middle][1];
...
static int gb2unicode(unsigned short *unicode,unsigned char*gb,int len) 
{ 
	int i = 0,j; 
	unsigned short new_gb = 0;
	...
	{ 
	  ...
	  { 
		...
		new_gb = (first_byte<<8)+second_byte;
		unicode[j]=binary_search(new_gb); 
		i+=2; 
	  } 
	} 
	return j*2; 
...
{ 
	...
	printf("\n");
#endif
	memset((unsigned char *)dst,0,256);
	if((current_size = gb2unicode(dst,src,len))==0xFFFF) 
	{ 
	  PRINTF("Failed\n");
	  return; 
	} 
	...
	{
		printf("%02X %02X ",(dst[i]>>8)&0xFF,dst[i]&0xFF);
	}
	printf("\n");
#endif	
	for(i=0; i<current_size/2; i++)
	{
		memset(utf8, 0, 3);
		ret = UnicodeToUTF8(utf8,(dst[i]>>8)&0xFF,dst[i]&0xFF);
		append_binary_to_array(buff, utf8, ret,num);
		num += ret;
	}
#if 0	
	printf("UTF-8  : ");
