...
extern "C" int set_gpio_value(int gpio, int value)
{ 
        int err = 0; 
        int fd; 
...
//		DEBUG_ERR("write_buf = 0x%x",write_buf[i]);
    ...
    unsigned long sub_addr, unsigned long rd_len, unsigned char*rd_buf)
{
	...
	int i;
//	for (i = 0;i<rd_len;i++)
//		DEBUG_ERR("rd_len = 0x%x",rd_buf[i]);


	const unsigned long sub_sub_adr = sub_addr;

//	printf(" sub_addr= 0x%x (uint8_t*)&sub_addr = 0x%x\n", sub_addr,*((uint8_t*)&sub_addr));
//	printf(" slave_addr= %x\n", slave_addr);
//	printf(" be_sub_addr= %x\n", be_sub_addr);
//	printf(" be_sub_addr&0xff= %x\n", (be_sub_addr&0xff));
	msg[0].addr = slave_addr;
	msg[0].flags = 0; //7 bits
...
//	for(i=0;i<msg[0].len ;i++) 
//			DEBUG_ERR(" %02X", msg[0].buf[i] );
//	printf(" sub_addr= 0x%x (uint8_t*)&sub_addr = 0x%x g_bAddressOffset = %d\n", sub_addr,*((uint8_t*)&sub_addr),g_bAddressOffset);
	for(i=0; i<g_bAddressOffset;i++){
	//printf("(unsigned char)(sub_addr>>((g_bAddressOffset-i-1)*8) =  0x%x\n",(unsigned char)(sub_sub_adr>>((g_bAddressOffset-i-1)*8)));
		msg[0].buf[i] = (unsigned char)(sub_sub_adr>>((g_bAddressOffset-i-1)*8));
		}
...
//			DEBUG_ERR(" rd_buf = %02X", msg[1].buf[i] );
...
//	for (i = 0;i < 2;i++)
//		printf("msg[i].addr = 0x%x msg[i].flags = 0x%x msg[i].len = 0x%x\n",msg[i].addr ,msg[i].flags  ,msg[i].len);
