...
char g_devname[MAX_DEV_NAME]="/dev/i2c-1";
...
static int getfwver(void)
{
	...
	if(dev)
	{
		...
		{
			for (i = 0;i < 4; i++)
				ver_buf[i] = cmd.data[i];
				
			char buf[sizeof(ver_buf) * 2 + 1 ];
			char *p=buf;
			for(i=0;i<sizeof(ver_buf);i++) {
      		p+=sprintf(p,"%d",255&ver_buf[i]);
			}
		  
		 if ((fd = open("/tmp/conex_fw_ver", O_WRONLY | O_CREAT | O_TRUNC, S_IREAD | S_IWRITE)) == -1)
  	 	 {
...
uint32_t  byte_swap_u32(uint32_t i)
{
...
static int get_mode()
{
	...
	cmd.data[0] = byte_swap_u32(cmd.data[0]);
	ret = SendCmdV(&cmd);
...
static int help(void)
{
	...
	printf("  -t, --test               	CxDish test mode\n");
...
int test_write( unsigned char slave_addr, 
    unsigned long sub_addr, unsigned char value)
{
	int ret;
	int fd ;
	...
	e2prom_data.nmsgs = 1;//因为都时序要两次，所以设为2  
	...
	printf("%s:%d slave_addr=0x%x\n",__func__, __LINE__,slave_addr);  
	/*read data from e2prom*/  
    e2prom_data.nmsgs = 1;//读时序要两次过程，要发两次I2C消息  
    e2prom_data.msgs[0].len = 2;//信息长度为1，第一次只写要读的eeprom中存储单元的地址  
    e2prom_data.msgs[0].addr = slave_addr;  
    e2prom_data.msgs[0].flags = 0;//写命令，看读时序理解  
    e2prom_data.msgs[0].buf = (uint8_t*)malloc(2);  
    e2prom_data.msgs[0].buf[0] = sub_addr;//信息值  
    e2prom_data.msgs[0].buf[1] = value;//信息值        
...
//    printf("write %02x from e2prom address %02x\n",e2prom_data.msgs[1].buf[0], sub_addr);  
  	close(fd);   
	return ret;
}

int test_read( unsigned char slave_addr, 
    unsigned long sub_addr)
{
    ...
    e2prom_data.msgs[0].addr = slave_addr;  
    e2prom_data.msgs[0].flags = 0;//写命令，看读时序理解  
    e2prom_data.msgs[0].buf = (uint8_t*)malloc(1);  
    e2prom_data.msgs[0].buf[0] = sub_addr;//信息值  
          
    e2prom_data.msgs[1].len = 1;  
    e2prom_data.msgs[1].addr = slave_addr;  
    e2prom_data.msgs[1].flags = I2C_M_RD;//读命令  
    e2prom_data.msgs[1].buf = (uint8_t*)malloc(1);  
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		if ((c = getopt_long(argc, argv, "qd:ghvsD:a:r:t:W:R:LH", long_option, NULL)) < 0)
			break;
		switch (c) {
		case 'h':
			help();
			int ret;
			read_addr = 0x5a;
			printf( "%s:%d  g_bChipAddress=0x%x, read_addr=0x%x\n",__func__,__LINE__, g_ChipAddress, read_addr);
		//	ret = test_read(g_ChipAddress,read_addr);
			ret = test_write(g_ChipAddress,read_addr, 0x64);
	...
	if(g_test) {
		...
		//if(dev)
		{
			...
			printf( "%s:%d  g_bChipAddress=0x%x, read_addr=0x%x\n",__func__,__LINE__, g_ChipAddress, read_addr);
			//ret = test_read(g_ChipAddress,read_addr);
			ret = test_write(g_ChipAddress,read_addr, 100);
			printf( "%s:%d ret = %d\n",__func__,__LINE__, ret);
			ret = test_read(g_ChipAddress,read_addr);
			printf( "%s:%d ret = %d\n",__func__,__LINE__, ret);
