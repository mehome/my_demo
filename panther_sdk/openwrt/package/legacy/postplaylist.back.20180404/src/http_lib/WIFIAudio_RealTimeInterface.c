...
//printf("%p     ", pStringObject);
//printf("%p     ", json_object_object_get(pObject, pKey));
//经过测试json_object_object_get两次地址是相同的，不会额外分配内存，是直接获取母对象的，也就无需手动调用释放json_object_put

//printf("%p     ", json_object_get_string(pStringObject));
//printf("%p     ", json_object_get_string(pStringObject));
//连续调用两次json_object_get_string地址也是相同的，相当于还是直接获取内容，并没有去分配内存

//连续调用两次json_object_to_json_string地址相同，并没有分配内存
...
//统一压缩int成Json的int对象，并添加挂载到JsonObject,不判断是否为宏定义错误值
...
/*
char * WIFIAudio_RealTimeInterface_pGetLoginAmazonNeedInfoTopString(WARTI_pGetLoginAmazonNeedInfo p)
{
	char *ret;
	struct json_object *object;
	...
	object = json_object_new_object();
	WIFIAudio_RealTimeInterface_StingToJsonStringAddJsonObject(object, "ret", p->pRet);
	WIFIAudio_RealTimeInterface_StingToJsonStringAddJsonObject(object, "ProductId", p->pProductId);
	WIFIAudio_RealTimeInterface_StingToJsonStringAddJsonObject(object, "Dsn", p->pDsn);
	WIFIAudio_RealTimeInterface_StingToJsonStringAddJsonObject(object, "CodeChallenge", p->pCodeChallenge);
	WIFIAudio_RealTimeInterface_StingToJsonStringAddJsonObject(object, "CodeChallengeMethod", p->pCodeChallengeMethod);
...
//		case WARTI_STR_TYPE_GETLOGINAMAZONNEEDINFO:
//			pBuf = WIFIAudio_RealTimeInterface_pGetLoginAmazonNeedInfoTopString(pStr->Str.pGetLoginAmazonNeedInfo);
//			break;
