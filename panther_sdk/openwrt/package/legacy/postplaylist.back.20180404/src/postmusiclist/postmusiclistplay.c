...
int WIFIAudio_PostMusicListPlay_PlayMusicList(void)
{
	int ret = 0;
	int fdfifo = 0;
	...
	if(fdfifo > 0)
	{
		...
		if(NULL != pfifobuff)
		{
			...
			if(NULL != pcmdparm)
			{
				pcmdparm->Command = WIFIAUDIO_FIFOCOMMUNICATION_COMMAND_MUSICLISTPALY;
...
int main(void)
{
	...
	if (NULL == lenstr)
	{
		...
		if(NULL == (fp = fopen(WIFIAUDIO_POSTPLAYLIST_PATHNAME, "w+")))
		{
			WIFIAudio_RetJson_retJSON("FAIL");
		} else {
			...
			if(!(ret < len))
			{
				ret = WIFIAudio_PostMusicListPlay_PlayMusicList();
				if(0 == ret)
				{
