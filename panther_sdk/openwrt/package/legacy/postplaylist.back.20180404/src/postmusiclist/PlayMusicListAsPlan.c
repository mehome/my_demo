...
extern char * WIFIAudio_RealTimeInterface_pGetCurrentPlaylistTopString(WARTI_pMusicList p);
...
int main(void)
{
	...
	if (lenstr == NULL)
	{
		...
		if(poststr == NULL)
		{
			...
			if(ret < len)
			{
				...
				pstr = WIFIAudio_RealTimeInterface_newStr(WARTI_STR_TYPE_PLAYMUSICLISTASPLAN, poststr);
	...
	if(pstr != NULL)
	{
		if(pstr->Str.pPlanPlayMusicList->pContent == NULL)
		{
			WIFIAudio_RetJson_retJSON("FAIL");
		} else {
			pstr->Str.pPlanPlayMusicList->pContent->NowPlay = 0;
			
			liststr = WIFIAudio_RealTimeInterface_pGetCurrentPlaylistTopString(pstr->Str.pPlanPlayMusicList->pContent);
			if(liststr == NULL)
			{
				WIFIAudio_RetJson_retJSON("FAIL");
			} else {
				memset(tmp, 0, sizeof(tmp));
				sprintf(tmp, WIFIAUDIO_TMPPLAYLISTASPLAN_PATHNAME, pstr->Str.pPlanPlayMusicList->Num);
				...
				if(ret > 0)
				{
					...
					sprintf(cmd,"alarm musiclist %d %d %02d %02d %02x \"%s\"", \
