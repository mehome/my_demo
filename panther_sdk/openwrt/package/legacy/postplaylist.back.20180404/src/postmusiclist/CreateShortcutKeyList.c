...
int WIFIAudio_CreateShortcutKeyList_CreateShortcutKey(void)
{
	int ret = 0;
	int fdfifo = 0;
	...
	if(fdfifo > 0)
	{
		...
		if(NULL != pfifobuff)
		{
			...
			if(NULL != pcmdparm)
			{
				pcmdparm->Command = WIFIAUDIO_FIFOCOMMUNICATION_COMMAND_CREATESHORTCUTKEYLIST;
...
int main(void)
{
	...
	if (NULL == lenstr)
	{
		...
		if(NULL == (fp = fopen(WIFIAUDIO_TMPSHORTCUTKEYLIST_PATHNAME, "w+")))
		{
			WIFIAudio_RetJson_retJSON("FAIL");
		} else {
			...
			if(!(ret < len))
			{
				ret = WIFIAudio_CreateShortcutKeyList_CreateShortcutKey();
				if(0 == ret)
				{
