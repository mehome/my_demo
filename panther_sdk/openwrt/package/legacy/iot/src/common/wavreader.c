...
void convert_16K_to_8K(unsigned char *buf,unsigned int * len)
{
    int i = 0,j = 0;
    unsigned short *sample = (short *)buf;
...
int wav_read_data(void* obj, unsigned char* data, unsigned int length) {
    ...
    convert_16K_to_8K(data,&n);
	return n;
}

int wav_read_data1(void* obj, unsigned char* data,  size_t size, size_t count) 
{
	size_t length ;
	length =  size*count;
	struct wav_reader* wr = (struct wav_reader*) obj;
	...
	if (length > wr->data_length)
		length = wr->data_length;
	n = fread(data, size, count, wr->wav);
	wr->data_length -= length;
	return n;
}
