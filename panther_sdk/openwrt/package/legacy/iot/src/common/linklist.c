...
/**
 * @file linklist.c Linking List help function
 *
 * @author  
 * @date   
 */
 ...
 *  @return none
 */
void inline listlink_add(struct list_head *new, struct list_head *prev, struct list_head *next)
{
    next->prev = new;
    new->next = next;
    new->prev = prev;
...
{
    listlink_add(new, head->prev, head);
}
