...
static void write_int16(struct wav_writer* ww, int value) {
 ...
 void write_header(struct wav_writer* ww, int length) {
...
void wav_write_data1(void* obj, const unsigned char* data,  size_t size, size_t count) {
	struct wav_writer* ww = (struct wav_writer*) obj;
	if (ww->wav == NULL)
		return;
	fwrite(data, size, count, ww->wav);
	ww->data_length += size*count;
}
