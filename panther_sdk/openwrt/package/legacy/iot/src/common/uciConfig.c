...
char * WIFIAudio_UciConfig_SearchValueString(char *psearch)
{
	...
	//pthread_mutex_lock(&g.ReadUCI_mtx);
    ...
    //pthread_mutex_unlock(&g.ReadUCI_mtx);
...
int WIFIAudio_UciConfig_SearchAndSetValueString(char *psearch, char *pstring)
{
	...
	//pthread_mutex_lock(&g.writeUCI_mtx);
    ...
    //pthread_mutex_unlock(&g.writeUCI_mtx);
	return ret;
}

int WIFIAudio_UciConfig_SearchAndSetValueStringNoCommit(char *psearch, char *pstring)
{
	...
	uci_set_confdir(ctx, WIFIAUDIO_CONFIG_PATH);

	char *psearchcopy = malloc(512);

	sprintf(psearchcopy, "%s=%s", psearch, pstring);

	if (uci_lookup_ptr(ctx, &ptr, psearchcopy, true) != UCI_OK) {
		goto end;
	}
