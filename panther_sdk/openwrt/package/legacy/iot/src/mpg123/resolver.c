...
static int timeout_connect(int sockfd, const struct sockaddr *serv_addr, socklen_t addrlen)
{
	if(timeout > 0)
	{
		...
		else if(errno == EINPROGRESS)
		{
			...
			if(err > 0)
			{
				...
				{
					error("connection error: %s", strerror(err));
					return -1;
				}
			}
			else if(err == 0)
			...
			{
				error("error from select(): %s", strerror(errno));
				return -1;
			}
		}
		else
	...
	else
	{
		...
		{
			error("connecton failed: %s", strerror(errno));
			return -1;
		}
...
#define INADDR_NONE 0xffffffff
	...
	debug("Note: Attempting new-style connection to %s\n", host->p);
	...
	if(sock < 0) 
        error("Cannot resolve/connect to %s:%s!", host->p, port->p);
