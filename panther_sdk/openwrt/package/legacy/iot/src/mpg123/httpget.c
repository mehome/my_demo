...
{
	int err;
	//debug("Attempting readstring on %d for %"SIZE_P" bytes", fd, (size_p)maxlen);
	string->fill = 0;
...
void get_header_string(mpg123_string *response, const char *fieldname, mpg123_string *store)
{
	...
	if((tmp = get_header_val(fieldname, response)))
	{
		...
		{
			debug("got %s %s", fieldname, store->p); return; 
		}
		else
		{ 
			error("unable to set %s to %s!", fieldname, tmp); 
		}
	}
}
...
size_t accept_length(void)
{
	...
	l += strlen("*/*\r\n");
	debug("initial computation of accept header length: %lu", (unsigned long)l);
	return l;
}

static char *proxy  = "http";
...
int proxy_init(struct httpdata *hd)
{
	...
	if (proxy && proxy && strcmp(proxy, "none"))
	{
		mpg123_string proxyurl;
		mpg123_init_string(&proxyurl);
		if(   !mpg123_set_string(&proxyurl, proxy)
		   || !split_url(&proxyurl, NULL, &hd->proxyhost, &hd->proxyport, NULL))
		{
			...
			fprintf(stderr, "Note: Using proxy %s\n", hd->proxyhost.p);
#if 0 /* not yet there */
...
{
	debug("request_url:%s", request_url->p);
	...
	debug("prefix=%s", purl->fill ? purl->p : "");
	if(!mpg123_add_string(purl, response->p+10)) return FALSE;

	debug("           purl: %s", purl->p);
	debug("old request_url: %s", request_url->p);
...
int http_open(char* url, struct httpdata *hd)
{
	...
	*/
	...
	do
	{
		...
		debug("attempting to open_connection to %s:%s", host.p, port.p);
		sock = open_connection(&host, &port);
		if(sock < 0)
		{
			error("Unable to establish connection to %s", host.fill ? host.p : "");
			goto exit;
		}
...
#define safe_readstring \
		...
		do
		{
			...
			{ /* We got a header line (or the closing empty line). */
				char *tmp;
				debug("searching for header values... %s", response.p);
				...
				if((tmp = get_header_val("icy-metaint", &response)))
				{
					...
					debug("got icy-metaint %li", (long int)hd->icy_interval);
				}
			}
	...
	if(relocate)
	{
		...
		error("Too many HTTP relocations (%i).", numrelocs);
