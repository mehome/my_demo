...
void free_playlist()
{
	if(pl.list != NULL)
	{
		...
		{
			--pl.fill;
			debug("free()ing entry %lu", (unsigned long)pl.fill);
...
int add_next_file (char *item)
{
	...
	if (item)
	{
		...
		if ((slashpos=strrchr(item, '/')))
		{
			/* up to and including /, with space for \0 */
			if(mpg123_resize_string(&pl.dir, 2 + slashpos - item))
			{
				memcpy(pl.dir.p, item, pl.dir.size-1);
	...
	if (item || pl.file)
	{
...

		{
			...
			else if (!strncmp(item, "http://", 7))
			{
				int fd;
				struct httpdata htd;
				httpdata_init(&htd);

				fd = http_open(item, &htd);

				debug("htd.content_type.p: %p", (void*) htd.content_type.p);
				if(htd.content_type.p != NULL)
				{
					int mimi;
					debug("htd.content_type.p value: %s", htd.content_type.p);
					...
					else
					{
						...
						if(mimi & IS_FILE)
						{
							...
							add_to_playlist(item, 0);
							return 1;
						
						}
						error("Unknown playlist MIME type %s; maybe "PACKAGE_NAME" can support it in future if you report this to the maintainer.", htd.content_type.p);
					}
					httpdata_free(&htd);
				}
				if(fd < 0)
			...
			*/
			...
			do
			{
				...
				if(fgets(pl.linebuf.p+have, pl.linebuf.size-have, pl.file))
				{
					...
					debug("have read %lu characters into linebuf: [%s]", (unsigned long)have, pl.linebuf.p);
				}
				else
				{
			...
			if(have)
			{
				...
				if(firstline)
				{
					if(pl.type == UNKNOWN)
					{
						...
						{
							debug("Note: detected M3U playlist type\n");
							pl.type = M3U;
						}
						else
						{
							debug("Note: guessed M3U playlist type\n");
							pl.type = M3U;
						}
					}
					else
				...
				if(pl.type == PLS)
				{
					if(!strncasecmp("File", pl.linebuf.p, 4))
					{
						...
						if((in_line = strchr(pl.linebuf.p+4, '=')) != NULL)
						{
							...
							{
								warning("Warning: Invalid PLS line (empty filename) - corrupt playlist file?\n");
								continue;
							}
						}
						else
						{
							warning("Warning: Invalid PLS line (no '=' after 'File') - corrupt playlist file?\n");
							continue;
						}
					}
					else
					{
						debug("#metainfo %s", pl.linebuf.p);
						continue;
					}
				}
