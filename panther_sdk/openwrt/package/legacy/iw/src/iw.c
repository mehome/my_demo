...
bool omnicfg_iw_scan_ssid_cmd;
bool omnicfg_iw_scan_ssid_found;
char omnicfg_iw_scan_ssid_name[40];
#endif
...
#if defined(CONFIG_OMNICFG)
        omnicfg_iw_scan_ssid_cmd = false;
        if (argc > 0 && strcmp(*argv, "--ssid") == 0)
        {
                omnicfg_iw_scan_ssid_cmd = true;
                argc--;
                argv++; 
                if (argc > 0)
                {
                        strcpy(omnicfg_iw_scan_ssid_name, *argv);
                        argc--;
                        argv++;
                } else {
                        omnicfg_iw_scan_ssid_cmd = false;
