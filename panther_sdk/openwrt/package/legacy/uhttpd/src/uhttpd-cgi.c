...
enum {MAX_AGE, DOMAIN, PATH, SECURE, VERSION, USERID};
static char *uh_cookie_attriblists[] =
{
	"Max-age=",
	"Domain=",
	"Path=",
...
static void uh_cgi_cookie_header_parse(struct http_response *pRes, char *buf)
{
	int i;
	char *ch = NULL;
	char *bufptr = NULL;
	
	for(int j = 0; j < COOKIE_MAX_LENGTH; j++)
	{
		pRes->setcookie[j] = buf[j];
		if( pRes->setcookie[j] == '\0' )
			break;
	}
	pRes->setcookie[COOKIE_MAX_LENGTH] = '\0';

	if((ch = strchr(pRes->setcookie, '=')) != NULL)
	{
		pRes->hc.name = pRes->setcookie;
		*ch++ = 0;
		pRes->hc.value = ch;
		while( (*ch != '\r') && (*ch != '\n') && (*ch != ';') )
			ch++;
	...
	for( i = 0; i < array_size(uh_cookie_attriblists); i++ )
	{
		if((ch = strstr(bufptr, uh_cookie_attriblists[i])) != NULL)
		{
			ch += strlen(uh_cookie_attriblists[i]);
			switch (i)
			{
				case MAX_AGE:
						pRes->hc.max_age = ch;
...
/*	printf("pRes->hc->name = %s\n",pRes->hc.name);
	printf("pRes->hc->value = %s\n",pRes->hc.value);
	printf("pRes->hc->max_age = %s\n",pRes->hc.max_age);
	printf("pRes->hc->domain = %s\n",pRes->hc.domain);
	printf("pRes->hc->path = %s\n",pRes->hc.path);
	printf("pRes->hc->secure = %s\n",pRes->hc.secure);
	printf("pRes->hc->version = %s\n",pRes->hc.version);*/
}

char *uh_gen_cookie_header(struct http_response *pRes)
{
	static char t[2048];

	strcpy(t,"");
	strcat(t,"%s: %s=%s;");
	if(pRes->hc.max_age != NULL)
		{
		strcat(t,"Max-age=");
		strcat(t,pRes->hc.max_age);
		strcat(t,";");
	}
	if(pRes->hc.domain != NULL)
			{
		strcat(t,"Domain=");
		strcat(t,pRes->hc.domain);
		strcat(t,";");
			}
	if(pRes->hc.path != NULL)
			{
		strcat(t,"Path=");
		strcat(t,pRes->hc.path);
		strcat(t,";");
						}
	if(pRes->hc.secure != NULL)
						{
		strcat(t,"Secure=");
		strcat(t,pRes->hc.secure);
		strcat(t,";");
						}
	if(pRes->hc.version != NULL)
					{
		strcat(t,"Version=");
		strcat(t,pRes->hc.version);
		strcat(t,";");
		}
	strcat(t,"\r\n");
...
#ifdef CONFIG_WEB_LOGIN
							if( ! strcasecmp(hdrname, "Set-Cookie"))
							{
								uh_cgi_cookie_header_parse(&res, bufptr);
							}
#endif
						}
...
#define ensure(x) \
	do { if( x < 0 ) goto out; } while(0)
			...
			while( 1 )
			{
						...
						if( (buflen = read(rfd[0], buf, sizeof(buf))) > 0 )
						{
							...
							if( ! header_sent )
							{
								...
								if( (res = uh_cgi_header_parse(hdr, hdrlen, &hdroff)) != NULL )
								{
									...
									#ifdef CONFIG_WEB_LOGIN
										if(!strcmp(res->headers[i], "Set-Cookie"))
										{
											uh_realm_set_cookie(cl, res, pi);
										}
									#endif
									}
								...
								else if( hdrlen >= sizeof(hdr) )
								{
									...
									snprintf(fixed_type, sizeof(fixed_type), uh_file_mime_lookup(pi->name), CHARSET);
										...
										"Pragma: no-cache\r\n"
										"Expires: -1\r\n"
										"%s\r\n",   req->version, fixed_type,
										(req->version > 1.0)? "Transfer-Encoding: chunked\r\n" : ""));
				...
				{
					/* ignore all EINTR(SIGCHLD) caused by our grand-children */
					if(++timeout_count <= cl->server->conf->script_timeout*2)
					{
						if(0==waitpid(child, NULL, WNOHANG))
							continue;
					}
