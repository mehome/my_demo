...
static bool uh_cgi_path_match(const char *prefix, const char *url)
{
	int len = strlen(prefix);

	if (strstr(url, "cli.cgi"))
		return true;

	if (strncmp(url, prefix, len) != 0)
		return false;

	if (strstr(url+len, ".cgi"))
		return true;
	else
		return false;
}
...
static bool check_cgi_path(struct path_info *pi, const char *url)
{
	...
	pi->ip = NULL;
#if defined(MONTAGE_UHTTPD)
	return uh_cgi_path_match(conf.cgi_docroot_path, pi->phys);
#else
