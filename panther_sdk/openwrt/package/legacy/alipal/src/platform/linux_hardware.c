...
char *platform_get_chipid(char chipid[PLATFORM_CID_LEN])
{
	...
	unsigned char chipid_wifi_mac[18] = {0};
	...
	//	sscanf(tmp, "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx", &val[0], &val[1], &val[2], &val[3], &val[4], &val[5]);
    	sprintf(chipid_wifi_mac, "%02x%02x%02x%02x%02x%02x", mac[0], mac[1], mac[2], mac[3], mac[4],mac[5]);
	}
	if (0 == strcmp(chipid_wifi_mac,"8c882b001187"))
		strncpy(chipid, "rtl8188eu 12345678", PLATFORM_CID_LEN);
	else
    	strncpy(chipid, "wi6600", PLATFORM_CID_LEN);
