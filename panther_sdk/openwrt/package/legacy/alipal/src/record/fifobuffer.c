...
int _ft_fifo_seek_command(FT_FIFO *fifo,
			   unsigned char *buffer, unsigned int offset)
{
	unsigned int l = 0,temp = 0;
	...
	while(__len < datasize) {
		data = fifo->buffer[temp];
		if(data ==' ' || data=='\r' || data=='\t' ||fifo->buffer[temp]=='\n')
		{
			//do nothing	
		}else {
	...
	if(find) 
	{
		//DEBUG_INFO("len:%d,__len:%d",len,__len );
		_ft_fifo_setoffset(fifo, __len);
		return len;
	} else 
	{
		return 0;
