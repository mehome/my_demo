...
static void airkiss_finish(struct mon_airkiss *monair)
{
    ...
    airkiss_output("ssid = \033[32m\"%s\"\033[0m, pwd = \033[32m\"%s\"\033[0m, ssid_length = %d, "
            "pwd_length = %d, random = 0x%02x\n",
            result.ssid, result.pwd, result.ssid_length,
            result.pwd_length, result.random);
    eval_nowait("aplay","-D","common","/tmp/res/receive_network_info.wav");
    ...
    monair->is_airkiss_complete = true;
    airkiss_output("mtc wlhup start");
    eval_nowait("mtc","wlhup");
    airkiss_output("mtc wlhup end");
#else
...
int main(int argc, char *argv[])
{
    ...
    else if (!strcmp("start", argv[1]))
    {
        int debug = 0;
        unsigned int state;
        cdb_set_int("$airkiss_start",2); //airkiss开始配网了
        state = cdb_get_int("$airkiss_state", AIRKISS_STANDBY);
