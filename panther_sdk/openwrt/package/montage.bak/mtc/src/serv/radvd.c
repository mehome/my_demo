...
*/
    ...
    if ((cdb_get_str("$lan6_ip", lan6_ip, sizeof(lan6_ip), NULL) == NULL) ||
        (!strcmp(lan6_ip, ""))){
        MTDM_LOGGER(this, LOG_ERR, "load args fail, lan6_ip=[%s]", lan6_ip);
        return 0;
    }
    ...
    exec_cmd2("ifconfig %s %s", mtdm->rule.LAN, lan6_ip);
    exec_cmd2("sysctl -w net.ipv6.conf.all.forwarding=1 > /dev/null 2> /dev/null");
    start_stop_daemon(RADVD_CMDS, SSDF_START_BACKGROUND_DAEMON, NICE_DEFAULT, 0, RADVD_PIDFILE, 
        "%s -C %s", RADVD_ARGS, RADVD_CONFILE);
...
static int stop(void)
{
    if( f_exists(RADVD_PIDFILE) ) {
        start_stop_daemon(RADVD_CMDS, SSDF_STOP_DAEMON, 0, SIGTERM, RADVD_PIDFILE, NULL);
