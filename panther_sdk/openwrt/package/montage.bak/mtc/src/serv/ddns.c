...
static int FUNCTION_IS_NOT_USED ddns_stop(void)
{
    if( f_exists(DDNS_PIDFILE) ) {
        start_stop_daemon(DDNS_CMDS, SSDF_STOP_DAEMON, 0, SIGKILL, DDNS_PIDFILE, NULL);
        unlink(DDNS_PIDFILE);
    }
    return 0;
}
...
//#$ddns_enable=1
    ...
    if (!ddns_enable) {
        MTDM_LOGGER(this, LOG_INFO, "ddns_enable=0");
        return 0;
    }

    if (cdb_get_str("$ddns_cfg1", ddns1, sizeof(ddns1), NULL) == NULL)
        goto err_args;
    ...
    (void) retry;
#endif
        start_stop_daemon(DDNS_CMDS, SSDF_START_BACKGROUND_DAEMON, NICE_DEFAULT, 0, DDNS_PIDFILE, 
            "%s", DDNS_ARGS);
    }
    else {
        ddns_stop();
        MTDM_LOGGER(this, LOG_INFO, "ddns en=0");
    }

    return 0;

err_args:
    MTDM_LOGGER(this, LOG_ERR, "load args fail, ddns1=[%s]", ddns1);
    return 0;
}
