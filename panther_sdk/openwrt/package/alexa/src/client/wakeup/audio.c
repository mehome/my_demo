      ...
      while (aud.fileIndex < aud.numFiles) {
        ...
        if (aud.sampleRate != SAMPLERATE) {
          sprintf(str, "Warning: Input sampleRate(%f) differs from defined sampleRate(%f). Skipping file (%s).", 
            (double)SAMPLERATE, (double)aud.sampleRate,aud.fssdk_input_file[aud.fileIndex - 1]);
          panic(aud.cons, str, thfGetLastError(aud.ses), 0);
          continue;
        }
        return 0;
...
int initAudio(HINSTANCE inst, void *c)
{
  ...
  if (!aud.bUseFileInput)
  {
      if ((r = waveInOpen(&aud.hwi, WAVE_MAPPER, &aud.f, (DWORD_PTR)aud.hwnd, 42,
...
static int openKnownAudio(int record) {
  ...
  if (record) {
    ...
    if ((rc=snd_pcm_open(&aud.inhandle, "plughw:2,0",SND_PCM_STREAM_CAPTURE, 0))<0) {
...
/*  frames = 32; 
  ...
  snd_pcm_hw_params_get_period_size(hw_params, &g.m_chunk_size, 0);
...
{ 
  if (!aud.bUseFileInput)
  {
	...
	{
		//stopRecord();
		printf("\033[0;32;31m""StartEngine...\n""\033[0m");
		MpdVolume(101);
		SetRecordFlag(0);
	...
	//printf("333333\n");
	  ...
	  snd_pcm_prepare(aud.inhandle);
    } else if(availSamples>=wantSamples) {
	//printf("444444\n");
      int gotSamples=0;
      //audiol_t *p=aud.last;
      //printf("want=%i avail=%i\n",wantSamples,availSamples);
	  if (availSamples - wantSamples > wantSamples)
			snd_pcm_forward(aud.inhandle, availSamples-wantSamples);
      //while (availSamples-gotSamples>=wantSamples) { // copy all avail buffers
        ...
        if ((rc = snd_pcm_readi(aud.inhandle,audiobuf,wantSamples)) < 0) {
			sprintf(err, "read failed (%s)\n", snd_strerror (rc));
			//audioPanic(err);
			return;
        }
		gotaudio = 1;
        gotSamples+=wantSamples;//p->len;
        //printf("gotSamples=%i\n",gotSamples);
		/*unsigned char *pp;
		unsigned char temp;
		unsigned char *buffer = audiobuf;//p->audio;
		int j;
		//for (j = 0;j < p->len*2; j ++){
		for (j = 0;j < wantSamples*2; j ++){
			pp = &buffer[j];
			temp = pp[0];
			pp[0] = pp[1];
			pp[1] = temp;
		}*/
		//fwrite(p->audio, p->len*sizeof(short), 1, fp);
		//snd_pcm_reset(aud.inhandle);
		//snd_pcm_prepare(aud.inhandle);
		//snd_pcm_forward(aud.inhandle, availSamples-gotSamples);
      //}	
    } else {
	//printf("555555\n");
...
{ 
  ...
  {
    char err[256];
    int wantSamples = AUDIO_BUFFERSZ * SAMPLERATE/1000;
    int availSamples=0;
    int rc;
	//int count = 0;
    if(!aud.recording) return;
	printf("111111\n");
	if (fread(audiobuf, 1, wantSamples*2, fp) != wantSamples*2)
		exit(0);
	//while(count < wantSamples*2)
	//{
	//	count += fread(audiobuf, 1, wantSamples*2 - count, fp);
	//}
