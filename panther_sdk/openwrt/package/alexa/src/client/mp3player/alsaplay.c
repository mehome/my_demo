...
ssize_t SNDWAV_WritePcm(unsigned char *data_buf, size_t wcount)  
{  
    ssize_t r;  
    ssize_t result = 0;  
...
//void alsa_init(void)
void alsa_init(int format, int rate, int channels)
{
...
#define PROMPT_TTS_FORMATA	SND_PCM_FORMAT_S16_LE
#define PROMPT_TTS_SPEED	16000
#define PROMPT_TTS_CHANNELS	1

static pthread_mutex_t PlayWavMtx = PTHREAD_MUTEX_INITIALIZER;  

void PlayWav(char *pPath)
{
	int nread = 0;
	
	pthread_mutex_lock(&PlayWavMtx);
	FILE * fp = NULL;
	...
	alsa_init(PROMPT_TTS_FORMATA, PROMPT_TTS_SPEED, PROMPT_TTS_CHANNELS);
	...
	pthread_mutex_unlock(&PlayWavMtx);
}

#if 0
extern FT_FIFO * g_DecodeAudioFifo;
void *play_audio_pthread(void)
{
	int iLength = 0;
	char *byTempBuffer = NULL;
	alsa_init();

	byTempBuffer = malloc(alsa_play_chunk_bytes);
	...
	while(1)
	{
		...
		if (iLength > 0)
		{
			...
			ft_fifo_seek(g_DecodeAudioFifo, byTempBuffer, 0, iLength);

			ft_fifo_setoffset(g_DecodeAudioFifo, iLength);

			printf("SNDWAV_Play_Buf...iLength:%d\n", iLength);
			SNDWAV_Play_Buf(byTempBuffer, iLength);

			memset(byTempBuffer, 0, alsa_play_chunk_bytes);
		}
		else
		{
			printf("no't have data..\n");
			usleep(100 * 1000);
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	from_fd = open("decoder.wav", O_RDONLY);

	wFilesize = get_file_size("decoder.wav");
	...
	playback_go(pData, wFilesize);
	//SNDWAV_Play_Buf(pData, wFilesize);
