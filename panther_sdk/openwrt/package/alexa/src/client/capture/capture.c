...
#define UPLOAD_FILE_NAME     "/tmp/16k.raw"
...
int capture_voice(void)
{
...
#ifdef ALEXA_FF
	OnWriteMsgToUartd(ALEXA_CAPTURE_START);
#endif
	...
	//thwparams.format = SND_PCM_FORMAT_S16_BE;
...
exit:
	...
	/*if (1 == g_byWakeUpFlag)
	{
		DEBUG_PRINTF("capture_voice Send to capture end ...");
...
{
	char str[256];
	myexec_cmd("uartdfifo.sh " ALEXA_CAPTURE_START);

	memset(str, 0, sizeof(str));
	sprintf(str, "(arecord -D plughw:1,0 -f s16_be -c 2 -d 8 -r 16000 %s) &", \
	UPLOAD_FILE_NAME);
...
static long capture(char *orig_name)
{
	...
	size_t cc = (rest <= (off64_t)chunk_bytes) ?
		(size_t)rest : chunk_bytes;
	
	size_t abc = cc * 8 / bits_per_frame;


	DEBUG_PRINTF("<capture>----chunk_size:%d, chunk_bytes:%d, abc:%d", chunk_size, chunk_bytes, abc);
	...
	while (1) 
	{
		...
		if (RECORD_VAD_END == GetVADFlag())
		{
			...
			{
				byFlag = 1;
				printf(LIGHT_RED"getRecord break ...sem_post Request_sem ..\n"NONE);
		...
		if ((endTime-startTime) >= CAPURE_TIMEOUT)
		{
			...
			{
				byFlag = 1;
				printf(LIGHT_RED"time out sem_post Request_sem ..\n"NONE);
				SendMSGToSendPthread();
			}
		...
		{
			ft_fifo_put(g_CaptureFifo, tmp, len);
			pAudioBuffer = tmp;
			wAudioLen = len;
		}
	...
	//	if (pcm_count >= POST_MAX_NUMBER)
		...
		if (0 == byFlag && pcm_count >= POST_MAX_NUMBER)
		{
			// 缓存4帧数据
			if (1 == GetAmazonConnectStatus())
			{
