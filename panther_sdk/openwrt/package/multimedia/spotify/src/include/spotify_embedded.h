 ...
 * \copyright Copyright 2015 Spotify AB. All rights reserved.
 */
 ...
 * \see SpConfig
 */
#define SP_MAX_OS_VERSION_LENGTH (30)
 ...
 * \see SpSetDeviceAlias, SpZeroConfGetVars, SpConnectionLoginZeroConf
 */
 ...
 * web APIs (https://developer.spotify.com/web-api/)
...
/**
 * \brief Maximum length of the account requirement string
 *        (not counting terminating NULL)
 * \see SpZeroConfVars
 */
#define SP_MAX_ACCOUNT_REQ_LENGTH (15)
 ...
 * (SpConfig::alias_attributes) to mark a device alias as representing a group.
 *
...
enum SpAPIReturnCode {
	/**
	 * \brief This code should be used when API call was successfull
	 */
	kSpAPINoError = 0,
	/**
...
enum SpPlaybackBitrate {
	 ...
	 */
	kSpPlaybackBitrateLow = 0,

	/**
	 * \brief Set bitrate to 160 kbit/s
	 */
	kSpPlaybackBitrateNormal = 1,

	/**
	 * \brief Set bitrate to 320 kbit/s
	 *
	 * This is the default bitrate. Call SpPlaybackSetBitrate() to change it.
	 */
	kSpPlaybackBitrateHigh = 2
};
...
enum SpConnectionNotification {
	 ...
	 *
	 * This notification will be sent when the connection to Spotify has been established after
	 * a prior #kSpConnectionNotifyDisconnect.
	 */
...
struct SpMetadata {
	 ...
	 *
	 * \note Applications can use the Spotify web APIs (https://developer.spotify.com/web-api/)
	...
	/**
	 * \brief Spotify URI of the original track before relinking (in the form "spotify:track:xxxxxxx...")
	 *
	 ...
	 *
	 * \note Typical use case would be: user adds track to the collecion, later on platform integration
	 * code wants to check if the current track is in the user collection. Due to track relinking
	 * the \ref track_uri might differ from the one that is saved in the collection previously. In order
...
struct SpZeroConfVars {
	...
	/**
	 * \brief String to be sent in the "activeUser" field of the "getInfo" response[*].
	 */
	char active_user[SP_MAX_USERNAME_LENGTH + 1];
	...
	/**
	 * \brief String to be sent in the "accountReq" field of the "getInfo" response[*].
	 */
	char account_req[SP_MAX_ACCOUNT_REQ_LENGTH + 1];
	...
	/**
	 * \brief Integer to be sent as string in the "resolverVersion" field of the "getInfo" response[*].
	 */
 ...
 * the function SpInit(), or invoke the function SpRegisterErrorCallback().
 *
 ...
 * in the callback.
 */
typedef void (*SpCallbackPlaybackNotify)(enum SpPlaybackNotification event,
					 void *context);
 ...
 * in the callback.
 */
typedef size_t (*SpCallbackPlaybackAudioData)(
 ...
 * in the callback.
 */
typedef void (*SpCallbackPlaybackApplyVolume)(uint16_t volume, void *context);

/**
 * \brief Callback to notify the application that it tried to play an unavailable track
 *
 * To register this callback, use the function SpRegisterPlaybackCallbacks().
 *
 * \param[in] track_uri Spotify URI of the track (in the form "spotify:track:xxxxxxx...") that cannot be played.
 * \param[in] context Context pointer that was passed when registering the callback
 *
 * \note The application should not block or call API functions that are not allowed
 * in the callback.
 *
 * \note The SDK will automatically attempt playing the next track.
 */
typedef void (*SpCallbackPlaybackUnavailableTrack)(const char *track_uri,
						   void *context);
 ...
 *
 * \param[out] preset_id The value specified to SpPresetSubscribe()
 * \param[out] playback_position Current playback time in milliseconds.  This
 *              must be saved persistently with the preset token, and specified
 *              when calling SpPlayPreset().
 * \param[out] buffer The buffer provided to SpPresetSubscribe(), filled with
 *              the binary preset token to save.  This may be NULL if an error
 *              has occurred.
 * \param[out] buff_size The number of binary bytes in \a buffer to save.
 * \param[out] error An error code if a problem occurred, or kSpErrorOk.
 * \param[out] context The context provided when the callback was registered.
 ...
 * callback.
 */
typedef uint8_t (*SpCallbackSavePreset)(int preset_id,
					uint32_t playback_position,
					const uint8_t *buffer, size_t buff_size,
					SpError error, void *context);
...
struct SpPlaybackCallbacks {
	...
	/** \brief Unavailable track callback */
	SpCallbackPlaybackUnavailableTrack on_unavailable_track;
 ...
 * in the callback.
 */
typedef void (*SpCallbackConnectionNotify)(enum SpConnectionNotification event,
					   void *context);
 ...
 * \param[in] device_alias The device alias that this credentials blob is valid for
 * \param[in] context Context pointer that was passed when registering the callback
 *
 * \note The application should not block or call API functions that are not allowed
 * in the callback.
 ...
 * in the callback.
 */
typedef void (*SpCallbackDebugMessage)(const char *debug_message,
				       void *context);
...
struct SpConfig {
	 ...
	 *
         * \deprecated Use SpConfig::client_id instead.
	 */
	 ...
	 *
	 * This string will be overridden by SpConfig::alias_names[0], if defined.
	 *
	 * SpInit() returns kSpErrorInvalidArgument if this is not specified.
	 */
	...
	/**
	 * \brief An array of UTF-8 encoded display names for the aliases of the application/device.
	 *
	 ...
	 *
	 * Switching between these aliases (by calling SpSetDeviceAlias(), or if using builtin
	 * ZeroConf, by its ZeroConf representation being selected for playback), would let an
	 * integration perform different audio routing depending on which alias
	 * was selected, without disrupting the current session or any ongoing streaming.
	 *
	 * If the builtin ZeroConf is enabled, the alias names needs to be defined by setting the
	 * pointers in the alias_names array to point to the display names to be used for each alias.
	 * The array must be filled from 0 and upwards uninterrupted - when encountering a NULL or
	 * empty string, no more aliases will be recognized.
	 *
	 * The strings will be truncated to at most SP_MAX_DISPLAY_NAME_LENGTH bytes
	 * (depending on the UTF-8-encoded characters), not counting the terminating NULL.
	 *
	 * Setting SpConfig::alias_names[0] will override SpConfig::display_name.
	 *
	 * \note \ref SpSetDisplayName also sets the display name, but only for the currently
	 * selected alias.
	 */
	const char *alias_names[SP_MAX_DEVICE_ALIASES];

	/**
	 * \brief An array of attributes for each device alias (as defined by \ref alias_names)
	 *
	 * Each entry in the alias_attributes array is an unsigned integer interpreted as a bitfield.
	 * The attributes for each device is OR:ed together and stored in the integer.
	 *
	 * Example:
         * \code{.c}
	 *   conf.alias_names[0] = "My group";
	 *   conf.alias_attributes[0] = SP_DEVICE_ALIAS_ATTRIBUTE_GROUP | SP_DEVICE_ALIAS_SOME_OTHER_ATTRIBUTE;
	 * \endcode
	 */
	uint32_t alias_attributes[SP_MAX_DEVICE_ALIASES];
	 ...
	 *
	 * This should be an ASCII string containing only letters, digits,
	 * "_" (underscore), "-" (hyphen) and "." (period). or NULL
	 *
	 * SpInit() returns kSpErrorInvalidArgument if this is invalid or longer than
	 ...
	 *        SP_ZEROCONF_DISABLED, SP_ZEROCONF_SERVE or
	 ...
	 *
	 * \note Only needs to be set if zeroconf_serve = SP_ZEROCONF_SERVE
	 * \else
	 * \brief <em>Not applicable in this eSDK configuration.</em>
	 * \endif
	 */
	 ...
	 *        assigned port if this is set to 0.
	 * \else
	 * \brief <em>Not applicable in this eSDK configuration.</em>
	 * \endif
	 */
 ...
 * \return Returns an error code
 */
SP_EMB_PUBLIC SpError
SpConnectionSetConnectivity(enum SpConnectivity connectivity);
 ...
 *
 * To get a credentials blob for the user, the application must implement the
 * Spotify Connect ZeroConf API. See the \ref ZeroConf manual.
 *
 ...
 * \param[in] device_alias The device alias number. If not using aliases, pass 0.
 * \return Returns an error code
 *
 ...
 * \see SpConnectionIsLoggedIn
 */
SP_EMB_PUBLIC SpError SpConnectionLoginBlob(const char *username,
					    const char *credentials_blob,
					    uint32_t device_alias);
 ...
 * \see SpConnectionIsLoggedIn
 */
SP_EMB_PUBLIC SpError SpConnectionLoginPassword(const char *username,
						const char *password);
 ...
 * \return Returns an error code
 *
 * \note When using device aliases, this will change the display name for
...
/**
 * \brief Make the device appear as a new and different device.
 *
 * This is useful if the device is part of several configured groups, which are
 * announced over ZeroConf. When one of these groups is selected to be active,
 * rather than reinitializing the eSDK from scratch with SpFree()/SpInit() to
 * set a new device ID, the device can appear under another alias instead.
 * The new name (if needed) is set with SpSetDisplayName().
 *
 * \param[in] alias An integer selecting an alias. Default is 0.
 * \return Returns an error code
 *
 * \note When using device aliases, this will change the group status for
 * the currently selected alias.
 */
SP_EMB_PUBLIC SpError SpSetDeviceAlias(uint32_t alias);

/**
 * \brief Return the currently selected device alias.
 *
 * \return The current selected device alias.
 */
SP_EMB_PUBLIC uint32_t SpGetDeviceAlias(void);
 ...
 */
SP_EMB_PUBLIC SpError
SpRegisterConnectionCallbacks(struct SpConnectionCallbacks *cb, void *context);
 ...
 * \return Returns an error code
 */
SP_EMB_PUBLIC SpError SpRegisterDebugCallbacks(struct SpDebugCallbacks *cb,
					       void *context);
 ...
 */
SP_EMB_PUBLIC SpError
SpRegisterPlaybackCallbacks(struct SpPlaybackCallbacks *cb, void *context);
 ...
 */
SP_EMB_PUBLIC SpError SpGetMetadata(struct SpMetadata *metadata,
				    int relative_index);
 ...
 * \note This API can be invoked from a callback.
 */
SP_EMB_PUBLIC SpError SpGetMetadataImageURL(const char *image_uri,
					    char *image_url,
 ...
 * \return Returns an error code
 */
SP_EMB_PUBLIC SpError SpPlaybackPlay(void);
 ...
 *
 * \param[in] enable 1 to enable, 0 to disable, 2 to enable repeat-1
 * \return Returns an error code
 *
 * The callback SpCallbackPlaybackNotify() will be called with the
 ...
 * received by SpCallbackPlaybackAudioData() does not change.
 *
 ...
 *       the notification #kSpPlaybackBecameInactive will be sent.
 ...
 */
SP_EMB_PUBLIC SpError SpPresetSubscribe(int preset_id, uint8_t *buffer,
					size_t buff_size);
 ...
 * \return Returns an error code
 */
SP_EMB_PUBLIC SpError SpPlayPreset(int preset_id, uint32_t playback_position,
				   uint8_t *buffer, size_t buff_size);

/**
 * \brief Set a preset to start playing at the given time.
 *
 * Tell eSDK to register a preset to play at a specific time, for use as an alarm
 * clock.  The alarm will be automatically resecheduled to play once every 24
 * hours until cancelled.
 *
 * The alarm clock is valid across logout / login cycles, and when the network is
 * disconnected.  In the case of network disconnection, or any other case when
 * the preset cannot be played, a beeping sound will be generated.  This does not
 * require network connectivity.  The alarm clock is registered for a device, not
 * for a particular Spotify user.
 *
 * The time must be specified as an offset from the current time, not a timestamp.
 * That is, time_offset is how many seconds from now the alarm should start.
 *
 * Accepts a preset blob, or NULL to play a default user-recommended preset.
 * See SpPlayPreset() for documentation on default presets.
 *
 * \note The preset blob does NOT have to remain valid after calling this function.
 *       eSDK will keep a copy internally.
 *
 * \note The volume will be increased to 1/4th of its range when the alarm sounds
 * if the volume is below that.
 *
 * \param[in] preset_id An integer indicating which preset this token is
 *              for.  For products with numbered presets, this should match the
 *              number of the preset button, starting from 1.  For use-cases
 *              with no integer mapping, this should be specified as -1.
 * \param[in] buffer Preset blob obtained with SpPresetSubscribe(), or NULL to
 *              use a default user-recommended preset.
 * \param[in] buff_size The size of \a buffer, or 0 if buffer is NULL.
 * \param[in] time_offset Number of seconds from now to start playback
 * \return Returns an error code
 */
SP_EMB_PUBLIC SpError SpSetAlarmClock(int preset_id, uint8_t *buffer,
				      size_t buff_size, uint32_t time_offset);

/**
 * \brief Cancel an alarm previously set with SpSetAlarmClock()
 *
 * This cancels an alarm scheduled with SpSetAlarmClock().  Note that this
 * function does not stop the audio playback if an alarm is currently active.
 *
 * \param[in] preset_id The preset ID passed when SpSetAlarmClock() was called
 * \return Returns an error code
 */
SP_EMB_PUBLIC SpError SpCancelAlarmClock(int preset_id);
 ...
 * \param[in] device_alias The device alias to retrieve variables for. If not using aliases,
 *              pass 0 here.
 *
 * \note This API can be invoked from a callback.
 */
SP_EMB_PUBLIC SpError SpZeroConfGetVars(struct SpZeroConfVars *vars, uint32_t device_alias);
 ...
 *              not counting the terminating NULL.
 * \param[in] device_alias The alias number associated with the ZeroConf instance
 *              that is invoking this function. If aliases aren't used, pass 0.
 * \return Returns an error code
 *
 * \note The login is performed asynchronously. The return value only indicates
 * whether the library is able to perform the login attempt. The status of
 ...
 * \see SpConnectionIsLoggedIn
 */
SP_EMB_PUBLIC SpError SpConnectionLoginZeroConf(const char *username,
						const char *zero_conf_blob,
