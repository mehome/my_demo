...
#ifdef QQ_COMPATIBLE
void output_set_pbmode(int mode, int loop) {
	if (output_module && output_module->set_pbmode) {
		output_module->set_pbmode(mode, loop);
	}
}

void output_switch_song(int idx) {
	if (output_module && output_module->switch_song) {
		output_module->switch_song(idx);
	}
}
#endif
