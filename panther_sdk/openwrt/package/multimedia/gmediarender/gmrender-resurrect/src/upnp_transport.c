...
#ifdef QQ_COMPATIBLE
static void upnp_transport_notify_hearbeart(void);
char *g_qplayid;
#endif
typedef enum {
...
#ifdef QQ_COMPATIBLE
				replace_var(TRANSPORT_VAR_CUR_MEDIA_DUR, tbuf);
#endif
...
static int stop(struct action_event *event)
{
	...
	if(g_qplayid)
	{
		upnp_transport_notify_hearbeart();
	}
#endif
	service_unlock();
...
#ifdef QQ_COMPATIBLE
	g_plist_lock();
	curidx = g_playlist.curidx;
	seekidx = g_playlist.seek_song_idx;
	g_plist_unlock();
	if(g_qplayid && seekidx > 0)
	{
		/* mplayer do not have playlist, it is buffer in libplayer
		* need to send command to libplayer to switch the correct idx
		* */
		output_switch_song(seekidx - curidx);
	}
#endif
		// Nothing to change.
		break;
...
static void upnp_transport_notify_hearbeart(void)
{
#define ADD_VAR_VALUE(v) \
        UPnPLastChangeBuilder_addByCollector(transport_service_.last_change, transport_variable_names[v], get_var(v))

	////ADD_VAR_VALUE(TRANSPORT_VAR_POS_REC_QUAL_MODE);
	ADD_VAR_VALUE(TRANSPORT_VAR_CUR_MEDIA_DUR); //no need
	ADD_VAR_VALUE(TRANSPORT_VAR_TRANSPORT_STATE);
	ADD_VAR_VALUE(TRANSPORT_VAR_CUR_TRACK_DUR); //no need
	////ADD_VAR_VALUE(TRANSPORT_VAR_REC_MEDIUM_WR_STATUS);
	////ADD_VAR_VALUE(TRANSPORT_VAR_REC_MEDIUM);
	ADD_VAR_VALUE(TRANSPORT_VAR_AV_URI);
	ADD_VAR_VALUE(TRANSPORT_VAR_TRANSPORT_STATUS);
	ADD_VAR_VALUE(TRANSPORT_VAR_CUR_TRANSPORT_ACTIONS);
	ADD_VAR_VALUE(TRANSPORT_VAR_CUR_TRACK_META);
	////ADD_VAR_VALUE(TRANSPORT_VAR_TRANSPORT_PLAY_SPEED);
	//ADD_VAR_VALUE(TRANSPORT_VAR_NR_TRACKS);
	//ADD_VAR_VALUE(TRANSPORT_VAR_AV_URI_META);
	////ADD_VAR_VALUE(TRANSPORT_VAR_PLAY_MEDIUM);
	ADD_VAR_VALUE(TRANSPORT_VAR_CUR_PLAY_MODE);
	////ADD_VAR_VALUE(TRANSPORT_VAR_PLAY_MEDIA);
	ADD_VAR_VALUE(TRANSPORT_VAR_CUR_TRACK);
	////ADD_VAR_VALUE(TRANSPORT_VAR_CUR_REC_QUAL_MODE);
	////ADD_VAR_VALUE(TRANSPORT_VAR_REC_MEDIA);
	ADD_VAR_VALUE(TRANSPORT_VAR_CUR_TRACK_URI);

	ADD_VAR_VALUE(TRANSPORT_VAR_REL_TIME_POS);
	UPnPLastChangeCollector_force_notify(transport_service_.last_change);
}
#endif
...
static int seek(struct action_event *event)
{
	...
	if(g_qplayid)
	{
		if(strcmp(unit, "TRACK_NR") == 0)
		{
			const char *target = upnp_get_string(event, "Target");
			g_plist_lock();
			g_playlist.seek_song_idx = atoi(target);
			g_plist_unlock();
		}
		upnp_transport_notify_hearbeart();
	}
...
#ifdef QQ_COMPATIBLE
static int setplaymode(struct action_event *event)
{
	if (obtain_instanceid(event, NULL) < 0) {
		return -1;
	}

	const char *playmode = upnp_get_string(event, "NewPlayMode");
	if(!playmode)
		return 0;
	...
	else if(strcmp(playmode, "REPEAT_TRACK"))
	{
#ifdef QQ_COMPATIBLE
		output_set_pbmode(PLAYER_PB_AUTO,PLAYER_LOOP_PLAYLIST);
#endif
		printf("[%s:%d]\n",__func__,__LINE__);
	}
	else if(strcmp(playmode, "REPEAT_ALL"))
	{
		printf("[%s:%d]\n",__func__,__LINE__);
	}
	else
...
#ifdef QQ_COMPATIBLE
	[TRANSPORT_CMD_SETPLAYMODE] =               {"SetPlayMode", setplaymode},	/* optional */
#endif
...
{
	struct playback_t *track = NULL;
	int idx=0;

	g_plist_lock();
	idx = g_playlist.curidx;
	g_plist_unlock();

	if(idx > 0)
		track = (struct playback_t *)&(g_playlist.plist[idx-1]);

	if(track)
	{
		char sidx[4]={0};
		sprintf(sidx, "%d", g_playlist.totalcnt);
		/*GetMediaInfo*/
		replace_var(TRANSPORT_VAR_NR_TRACKS, sidx);
		replace_var(TRANSPORT_VAR_AV_URI, g_qplayid);
		replace_var(TRANSPORT_VAR_AV_URI_META, track->metadata);

		/*GetPositionInfo*/
		sprintf(sidx, "%d", idx);
		replace_var(TRANSPORT_VAR_CUR_TRACK, sidx);
		replace_var(TRANSPORT_VAR_CUR_TRACK_META, track->metadata);
		replace_var(TRANSPORT_VAR_CUR_TRACK_URI, track->trackuri);

		g_plist_lock();
		if(g_playlist.start == 0)
		{
			/*For sync to the next song position (auto switch)*/
			g_playlist.start = 1;
			replace_var(TRANSPORT_VAR_REL_TIME_POS, kZeroTime);
		}
		g_plist_unlock();
	...
	else
	{
		...
		replace_var(TRANSPORT_VAR_CUR_MEDIA_DUR, kZeroTime);

		/*GetPositionInfo*/
		replace_var(TRANSPORT_VAR_CUR_TRACK, "0");
		replace_var(TRANSPORT_VAR_CUR_TRACK_META, "");
		replace_var(TRANSPORT_VAR_CUR_TRACK_URI, "");

		replace_var(TRANSPORT_VAR_CUR_TRACK_DUR, kZeroTime);
		replace_var(TRANSPORT_VAR_REL_TIME_POS, "");
