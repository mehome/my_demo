...
#ifdef QQ_COMPATIBLE
void UPnPLastChangeBuilder_addByCollector(upnp_last_change_collector_t *object,
			const char *name, const char *value)
{
	UPnPLastChangeBuilder_add(object->builder, name, value);
}

void UPnPLastChangeCollector_force_notify(upnp_last_change_collector_t *obj)
{
	char *xml_doc_string = UPnPLastChangeBuilder_to_xml(obj->builder);
	if (xml_doc_string == NULL)
		return;
