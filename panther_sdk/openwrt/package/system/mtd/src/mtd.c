...
int output_progress_file = 0;
int total_size = 0;
int init_size = 0;

#define DEFAULT_PROGRESS_FILE		"/tmp/progress"
#define DEFAULT_PROGRESS_FILE_TMP  	DEFAULT_PROGRESS_FILE".tmp"

void update_progress_file(ssize_t w)
{
    FILE *fp;
    int accumulate_size = init_size + w;
    float percentile;

    if (total_size > 0)
    {
        fp = fopen(DEFAULT_PROGRESS_FILE_TMP, "w");
        if (fp)
        {
            if (accumulate_size>total_size)
                accumulate_size = total_size;

            percentile = (((float) accumulate_size) / ((float) total_size)) * 100;
            fprintf(fp, "%2.1f%%", percentile);
            fclose(fp);

            fprintf(stderr, "   %02d%%\b\b\b\b\b\b", (int) percentile);

            rename(DEFAULT_PROGRESS_FILE_TMP, DEFAULT_PROGRESS_FILE);
        }
    }
}
...
mtd_write(int imagefd, const char *mtd, char *fis_layout, size_t part_offset)
{
	...
	envp = getenv("MTD_WRITE_TOTAL_SIZE");
	if(envp)
	{
		total_size = strtoul(envp, 0, 10);
		output_progress_file = 1;
	}

	envp = getenv("MTD_WRITE_INIT_SIZE");
	if(envp)
		init_size = strtoul(envp, 0, 10);
#endif
...
#if defined(__PANTHER__)
		if (output_progress_file)
			update_progress_file(w);
#endif
