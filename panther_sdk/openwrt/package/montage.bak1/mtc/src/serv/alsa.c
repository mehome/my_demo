...
static int set_duercfg(char *pName, int Vol)
{
	char *json_str=NULL;
	...
	json_object_object_add(obj,pName, json_object_new_int(Vol));

	char *attr=json_object_get_string(obj);
	if(!attr){
		err=-4;
		goto Err3;
	}
	err = fp_write_file("/root/appresources/dueros_config.json", attr, "w");
	if(err < 0){
		cslog("wirte failed");
		goto Err3;
	}
...
static int start(void){
	...
	err = get_duercfg("tip_volume", &vol);
	if(err<0){
		exec_cmd2("amixer set TIPSOUND %d > /dev/null",get_vol_set(PROMPT_VOL));
		set_duercfg("tip_volume", PROMPT_VOL);
	}else{
		exec_cmd2("amixer set TIPSOUND %d > /dev/null",get_vol_set(vol));
	}
