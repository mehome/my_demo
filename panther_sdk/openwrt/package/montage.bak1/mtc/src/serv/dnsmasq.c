...
#define DNS_ARGS        "-C /tmp/"DNS_NAME".conf -K -x "DNS_PIDFILE
...
int dnsmasq_save_conf(int dns_enable, int dhcps_enable)
{
    ...
    if (fp) {
                    ...
                    if (fpb) {
                        while ((ptr = new_fgets(buf, sizeof(buf), fpb)) != NULL) {
                            if (!strncmp(ptr, "nameserver", sizeof("nameserver")-1)) {
                                ...
                                if (!strncmp(ptr, RESOLV_SKIP, sizeof(RESOLV_SKIP)-1)) {
                                    continue;
                                }
                                if (!dns_default) {
                                    fprintf(fp, ",");
        ...
        if (dhcps_probe) {
            MTDM_LOGGER(this, LOG_INFO, "********************************************");
            MTDM_LOGGER(this, LOG_INFO, "* DHCP Server probe is not implemented yet *");
            MTDM_LOGGER(this, LOG_INFO, "********************************************");
        }
        ...
        MTDM_LOGGER(this, LOG_ERR, "Can't open %s", DNS_NEW_CONFILE);
        return 1;
    }

    if (cp(DNS_NEW_CONFILE, DNS_CONFILE) == 0) {
        unlink(DNS_NEW_CONFILE);
    }
    else {
        MTDM_LOGGER(this, LOG_ERR, "Can't move %s %s", DNS_NEW_CONFILE, DNS_CONFILE);
    }
...
static int start(void)
{
    ...
    start_stop_daemon(DNS_CMDS, SSDF_START_DAEMON, NICE_DEFAULT, 0, NULL, 
        "%s", args);
...
static int stop(void)
{
    if( f_exists(DNS_PIDFILE) ) {
        start_stop_daemon(DNS_CMDS, SSDF_STOP_DAEMON, 0, SIGKILL, DNS_PIDFILE, NULL);
        unlink(DNS_PIDFILE);
    }
    return 0;
}
