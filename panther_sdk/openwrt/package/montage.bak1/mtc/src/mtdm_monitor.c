...
// 1: alexa??¨®D??DD¡ê?D¨¨¨°a??DD 0¡êoalexa?y?¨²??DD¡ê?2?¨®?¡ä|¨¤¨ª
...
#if 0
    if (cdb_get_int("$ra_func", 0) == 2)
    {
		char cmd[32];
		struct stat st;
        
        memset(&st, 0 , sizeof(st));
        if ((stat("/playlists/CurList.m3u", &st) == 0)
            && (!S_ISDIR(st.st_mode)))
        {
            exec_cmd("mpc load CurList.m3u");
            snprintf(cmd, sizeof (cmd), "mpc play %d",
                     cdb_get_int("$mpd_lastplay", 1));
            exec_cmd(cmd);
		}
	}
#endif
