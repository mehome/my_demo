...
static char *recv_Des48_data(char *inBufhex ,char *uckey ,char *putBuf ,int count,int (*K_po)[56]);
static char  *send_Enc48_data(char *inBuf ,char *uckey ,char *putBuf ,int count,int (*K_po)[56]);
...
//	int fd = (int)arg;
	...
	while(1)
	{
		memset(read_buf,0, sizeof(read_buf)-1);
		memset(dec_buf,0,sizeof(dec_buf)-1);
		int rtv_time = Client_Select_TimeOver(fd_client,1,0);
		if(rtv_time > 0)	
		{
			while(1)
			{
			...
			//	memset(read_buf,0, sizeof(read_buf)-1);
				int rtv_read = read(fd_client, read_buf, 1024); 		
			//	int rtv_read = recv(fd_client, read_buf, 1024,0);
#if 1		
...
#endif
				printf("rtv_read:%d,-----read_buf: %s\r\n",rtv_read, read_buf); 				//获取服务器下发的消息
			}
		//	for(i=0;i<100;i++)
		//		printf("%2x,",read_buf[i]);
			
			recv_Des48_data(read_buf,loginkey,dec_buf,strlen(read_buf),K_po);	//解密接口函数
...
//	for ( j = 0;j <=48*count;j++)	printf("%02x", (unsigned char )putBuf[j] );
	printf("\n");
...
memset((void*)ucBufKey1, 0x00, sizeof(ucBufKey1));
memset((void*)ucBufKey2, 0x00, sizeof(ucBufKey2));
memset((void*)ucBufKey3, 0x00, sizeof(ucBufKey3));

for( j=0;j<8;j++) 
{
   ucBufKey1[j]=keyC[	j] ;
   ucBufKey2[j]=keyC[ 8+j] ;
   ucBufKey3[j]=keyC[16+j] ;
}
#endif
...
int main(int argc, char const *argv[])
{
	...
	unsigned char *rev_buf=NULL;
//	char *SendBufHex = NULL;
	if(argc != 1)
	{
		printf("Usage: %s <IP> <PORT>\n", argv[0]);
		exit(0);
	}
//	rev_buf = device_blank_request_data();
//	device_login_request_data(strdup("012345678912345"),strdup("460001234567890"),strdup("huawei"),strdup("v2.39"),strdup("v01.00"),4);
//	printf("rev_buf:%s\n",rev_buf);

	/*空白心跳*/
	unsigned char txt[ ] = "cmd_typ=heart_emp_send";//保存明文

//	unsigned char txt[ ] = "wd,00000000,460001711930796_861477032135319/249a";//保存明文249a


	/*数据心跳测试*/
//	unsigned char txt[ ] = "cmd_typ=heart_data_send,cmd_id=00000000000000000_00000005,dev_time=20181212010203,sig=31,net=3,tag_open=1,tag_move=1,cell_v=4.1,cell_pwr=35,lost_pwr=0";


	/*登录测试*/
//	unsigned char txt[ ] = "cmd_typ=log_req,cmd_id=00000000000000000_00000000,meid=012345678912345,imsi=460001234567890,brand=huawei,hardver=v2.39,softver=v01.00,net=4,sig=31";

#if 1	
	unsigned char pi_ucOut[48*4]={0};
	unsigned char pi[48*4]={0};
#else
	unsigned char pi_ucOut[48];
	unsigned char pi[48];
#endif
	...
	/*直接发登录的密码报文*/
//	strcpy(send_buf,"");
	printf("sizeof:%ld\r\n",strlen(txt));
	
//	char *txt = device_login_request_data(strdup("012345678912345"),strdup("460001234567890"));
//	char *txt = device_blank_request_data();
	char *SendBufHex = send_Enc48_data(txt,loginkey,pi_ucOut,strlen(txt),K_po);		//加密接口
	recv_Des48_data(SendBufHex,loginkey,pi,strlen(SendBufHex),K_po);			//解密接口
#if 0
	char outbuf[1024]={0};
	char inbuf[] = "e8a4b0d61ada47fda7addff0fa18091e493f";
	MD5Trans32string(inbuf,outbuf,strlen(inbuf));
	printf("MD5Trans32string outbuf:%s\n",outbuf);
#endif

//	cmdtype_log_rand_event(strdup("cmd_typ=log_rand,cmd_id=00000000000000000_00000001,random=e8a4b0d61ada47fda7addff0fa18091e493f,crc=43e2"));
//	cmdtype_log_resu_event(strdup("cmd_typ=log_resu,cmd_id=00000000000000000_00000003,result=0,crc=603e"));

	fd_client = create_socket_client();
	if(fd_client < 0)
	{
		printf("create_socket_client is failed \n");
		return 0;
	}
//	printf("fd_client:%d\n",fd_client);
//	CreateSocketWritePthread(fd_client);

	pthread_attr_t a_thread_attr; 
	pthread_attr_init(&a_thread_attr);
	pthread_attr_setstacksize(&a_thread_attr, PTHREAD_STACK_MIN);
	if(0 == pthread_create(&read_pthread, &a_thread_attr, Read_pthread, NULL))
	{
		pthread_detach(read_pthread);		//设置为分离属行，程序退出时，系统自动回收
	}
/*
	while(1)
	{
		...
		printf("SendBufHex:%s\n",SendBufHex);
		rtv = write(fd_client, SendBufHex, strlen(SendBufHex));
	//	rtv = write(fd_client, send_buf, strlen(send_buf));
	//	rtv = send(fd_client, send_buf, strlen(send_buf),0);
		if(rtv < 0)
		{
			perror("\033[1;32;40mclient write data failed ... \033[0m\n");
			break;
...
//	unsigned char tmp_buf2[] = "cmd_typ=heart_emp_send";
		...
		/*数据心跳测试*/
//	unsigned char tmp_buf2[ ] = "cmd_typ=heart_data_send,cmd_id=00000000000000000_00000005,dev_time=20181212010203,sig=31,net=3,tag_open=1,tag_move=1,cell_v=4.1,cell_pwr=35,lost_pwr=0";
...
//	char buf1[] ="538980ddfb87c6669aa55fce90ffcb02d30bf08c94b094565149bed614fa8f8f83211a28cf90a15bda69273188fbcedc";
